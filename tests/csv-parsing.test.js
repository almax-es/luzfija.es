...
describe('parseEnergyTableRows - columna INV/VER entrecomillada', () => {
  let csvUtils;
  beforeAll(() => { csvUtils = window.LF.csvUtils; });

  it('resuelve igual la hora repetida del cambio horario venga o no entrecomillada la columna INV/VER', () => {
    const rows = [
      ['FECHA', 'HORA', 'CONSUMO_KWH', 'INV_VER'],
      ['27/10/2024', '0', '9', '0'],   // fuerza hourBase=zero (hora cruda 0 => hora 1)
      ['27/10/2024', '2', '11', '0'],  // 1ª aparición de la hora repetida, INV/VER sin comillas
      ['27/10/2024', '2', '22', '"1"'] // 2ª aparición, INV/VER entrecomillado ("1")
    ];
    ...
    // Sin stripOuterQuotes, la comparación inv === '1' fallaría y la 2ª
    // aparición degradaría a hora 25 en vez de resolverse a hora 3 como indica
    // explícitamente la columna INV/VER="1".
    expect(primera.hora).toBe(3);
    expect(segunda.hora).toBe(3);
    expect(result.records.some((r) => r.hora === 25)).toBe(false);
  });
});
