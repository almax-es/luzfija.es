...
ES12345;01/01/2024;2;"1,234";Real`;
    ...
    expect(resultado.warnings.some(w => w.includes('superiores a 10.000 kWh'))).toBe(true);
  });
});
...
describe('parseEnergyTableRows - Datadis mensual', () => {
  ...
  it('Datadis mensual no se ve afectado por el fix de Ceuta/Melilla (regresion 14/08/2026)', () => {
    // Datadis sintetiza hora:1/11/12 -> P3/P2/P1 de forma incondicional, sin depender de
    // zonaFiscal; el fix de Ceuta/Melilla vive en parseEnergyTableRows/detectHourBase, un
    // camino de codigo distinto al que usa este formato.
    const rows = makeRows([['ES001', '2025/06', '100', '50', '30', '20', '80', '60', '']]);
    const result = csvUtils.parseEnergyTableRows(rows, { zonaFiscal: 'CeutaMelilla' });
    const periodos = result.records.map(r => r.periodo).sort();
    expect(periodos).toEqual(['P1', 'P2', 'P3']);
  });
  ...
  it.each([
    ['Valle', ['ES001', '2025/01', '-100', '50', '30', '20', '40', '60', '']],
    ['Llano', ['ES001', '2025/01', '100', '-50', '30', '20', '40', '60', '']],
    ['Punta', ['ES001', '2025/01', '100', '50', '-30', '20', '40', '60', '']],
    ['Vertida', ['ES001', '2025/01', '100', '50', '30', '-20', '40', '60', '']],
    ['Generada', ['ES001', '2025/01', '100', '50', '30', '20', '-40', '60', '']],
    ['Autoconsumida', ['ES001', '2025/01', '100', '50', '30', '20', '40', '-60', '']]
  ])('rechaza %s negativo (14/08/2026)', (_campo, fila) => {
    const rows = makeRows([fila]);
    expect(() => csvUtils.parseEnergyTableRows(rows)).toThrow();
  });
...
// ===== BV-IMPORT.JS: MATRIZ HORARIA CON HORA 25 (DST) =====

function buildMatrixHeaderRow({ withH25 = false } = {}) {
  const base = ['FECHA', ...Array.from({ length: 24 }, (_, i) => `H${String(i + 1).padStart(2, '0')}`)];
  return withH25 ? [...base, 'H25'] : base;
}

function mockXLSXWithData(data) {
  global.XLSX = {
...
describe('bv-import.js - matriz horaria E-REDES con hora 25 (DST)', () => {
  afterEach(() => {
    delete global.XLSX;
  });

  // OJO: la cabecera tiene que DECLARAR H25. Hasta el 25/07/2026 este test construia una
  // cabecera que solo llegaba a H24 y añadia un 25 valor suelto, con lo que congelaba el
  // bug de que cualquier columna final (p.ej. un total diario) entrase como hora 25.
  it('incluye la hora 25 cuando la cabecera declara H25 y la fecha es el cambio horario', async () => {
    const header = buildMatrixHeaderRow({ withH25: true });
    const dataRow = ['27/10/2024', ...Array.from({ length: 24 }, () => '1'), '5'];
    mockXLSXWithData([header, dataRow]);

    const file = new File(['dummy'], 'matriz.xlsx', {
      type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    });
    const result = await window.BVSim.importFile(file, 'Península');

    expect(result.ok).toBe(true);
    expect(result.records).toHaveLength(25);
    const hora25 = result.records.find((r) => r.hora === 25);
    expect(hora25).toBeDefined();
    expect(hora25.kwh).toBe(5);
  });

  it('ignora H25 vacía/null sin romper los 24 registros normales', async () => {
    const header = buildMatrixHeaderRow();
    // Sin columna 25: la fila termina en H24 (índice 24), igual que exportan
    // muchas distribuidoras fuera del cambio de hora de octubre.
    const dataRowSinH25 = ['27/10/2024', ...Array.from({ length: 24 }, () => '1')];
    mockXLSXWithData([header, dataRowSinH25]);

    const file = new File(['dummy'], 'matriz-sin-h25.xlsx', {
      type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    });
    const result = await window.BVSim.importFile(file, 'Península');
...
// ===== INV/VER ENTRECOMILLADO (resolución DST en formato hora 0-23) =====

describe('parseEnergyTableRows - cambio de octubre en formato 0-23', () => {
  let csvUtils;
  beforeAll(() => { csvUtils = window.LF.csvUtils; });

  it('usa INV/VER para separar las dos ocurrencias de la hora 2 peninsular', () => {
    const rows = [
      ['FECHA', 'HORA', 'CONSUMO_KWH', 'INV_VER'],
      ['27/10/2024', '0', '9', '0'],   // fuerza hourBase=zero (hora cruda 0 => hora 1)
      ['27/10/2024', '2', '11', '1'],   // 1ª aparición: horario de verano
      ['27/10/2024', '2', '22', '"0"'] // 2ª aparición: horario de invierno, entrecomillado
    ];

    const result = csvUtils.parseEnergyTableRows(rows, { headerRowIndex: 0 });

    const primera = result.records.find((r) => r.kwh === 11);
    const segunda = result.records.find((r) => r.kwh === 22);

    expect(primera).toBeDefined();
    expect(segunda).toBeDefined();
    expect(primera.hora).toBe(3);
    expect(segunda.hora).toBe(25);
  });

  it('usa la hora 1 como repetida en Canarias', () => {
    const rows = [
      ['FECHA', 'HORA', 'CONSUMO_KWH', 'INV_VER'],
      ['27/10/2024', '0', '9', '1'],
      ['27/10/2024', '1', '11', '1'],
    ...
    expect(result.records.find((r) => r.kwh === 11).hora).toBe(2);
    expect(result.records.find((r) => r.kwh === 22).hora).toBe(25);
  });

  it('desambigua por orden una fecha-hora repetida aunque no exista INV/VER', () => {
    const rows = [
    ...
    expect(result.records.find((r) => r.kwh === 11).hora).toBe(3);
    expect(result.records.find((r) => r.kwh === 22).hora).toBe(25);
  });
});
...
describe('Deteccion de periodos duplicados (14/08/2026)', () => {
  ...
  it('rechaza fecha+hora duplicada fuera de cualquier cambio de hora', () => {
    const rows = [
      ['FECHA', 'HORA', 'CONSUMO_KWH'],
      ['01/06/2026', '5', '1'],
      ['01/06/2026', '5', '1']
    ];
    expect(() => csvUtils.parseEnergyTableRows(rows, { headerRowIndex: 0 })).toThrow(/duplicad/i);
  });

  it('una fila invalida no bloquea una fila valida posterior con la misma fecha/hora', () => {
    const rows = [
      ['FECHA', 'HORA', 'CONSUMO_KWH'],
      ['01/06/2026', '5', 'texto-no-numerico'],
      ['01/06/2026', '5', '3']
    ];
    const result = csvUtils.parseEnergyTableRows(rows, { headerRowIndex: 0 });
    expect(result.records).toHaveLength(1);
    expect(result.records[0].kwh).toBe(3);
  });

  it('rechaza una H25 duplicada de verdad (dos filas marcadas invierno para la misma fecha)', () => {
    const rows = [
      ['FECHA', 'HORA', 'CONSUMO_KWH', 'INV_VER'],
      ['27/10/2024', '0', '9', '0'],
      ['27/10/2024', '2', '11', '0'],
  ...
  it('rechaza un dia duplicado en formato matriz', () => {
    const HDR = ['Fecha', ...Array.from({ length: 24 }, (_, i) => 'H' + String(i + 1).padStart(2, '0'))];
    const fila = ['01/06/2026', ...Array(24).fill('1')];
    expect(() => csvUtils.parseHourlyMatrixRows([HDR, fila, fila.slice()], 0)).toThrow(/duplicad/i);
  });

  it('fila matriz sin registros validos no bloquea una fila valida posterior del mismo dia', () => {
    const HDR = ['Fecha', ...Array.from({ length: 24 }, (_, i) => 'H' + String(i + 1).padStart(2, '0'))];
    const filaInvalida = ['01/06/2026', ...Array(24).fill('99999')]; // fuera de rango, 0 registros
    const filaValidaMismoDia = ['01/06/2026', ...Array(24).fill('1')];
    const filaValidaOtroDia = ['02/06/2026', ...Array(24).fill('1')]; // dilucion, evita el umbral 50% invalido/no-vacio
    const result = csvUtils.parseHourlyMatrixRows(
      [HDR, filaInvalida, filaValidaMismoDia, filaValidaOtroDia],
      0
    );
    expect(result.records.filter((r) => r.fecha.getDate() === 1)).toHaveLength(24);
  });

  it('rechaza un mes duplicado en formato Datadis mensual', () => {
    const header = ['CUPS', 'Fecha', 'Valle', 'Llano', 'Punta', 'Energia_vertida_kWh', 'Energia_generada_kWh', 'Energia_autoconsumida_kWh', 'Consumo_Anual'];
    const rows = [
      header,
      ['ES001', '2025/01', '100', '50', '30', '20', '80', '60', ''],
      ['ES001', '2025/01', '100', '50', '30', '20', '80', '60', '']
    ];
    expect(() => csvUtils.parseEnergyTableRows(rows)).toThrow(/duplicad/i);
  });

  it('un mes Datadis invalido no se registra como visto: el error sigue siendo el original', () => {
    const header = ['CUPS', 'Fecha', 'Valle', 'Llano', 'Punta', 'Energia_vertida_kWh', 'Energia_generada_kWh', 'Energia_autoconsumida_kWh', 'Consumo_Anual'];
    const rows = [
      header,
      // Generada (0) != Vertida (1) + Autoconsumida (1) -> lanza 'inconsistentes' antes
      // de llegar a la comprobacion de mes duplicado.
      ['ES001', '2025/01', '100', '50', '30', '1', '0', '1', ''],
      ['ES001', '2025/01', '100', '50', '30', '20', '80', '60', '']
    ];
    expect(() => csvUtils.parseEnergyTableRows(rows)).toThrow(/inconsistentes/i);
  });
