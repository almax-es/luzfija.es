...
describe('repo docs metrics', () => {
  ...
  it('valida ejecuciones completas por archivos de test y no por bloques describe', () => {
    const syncScript = fs.readFileSync(path.join(repoRoot, 'scripts/sync-seo-docs.mjs'), 'utf8');

    expect(syncScript).toContain('summary.testResults.length === files.length');
    expect(syncScript).not.toContain('summary?.numTotalTestSuites === files.length');
  });
});
