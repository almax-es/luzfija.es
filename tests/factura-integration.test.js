...
// Espera a que termine el procesado de la factura. `__LF_FACTURA_BUSY` se activa de forma sincrona
// al elegir el fichero (o pulsar OCR) y se libera en el `finally` de la operacion. Sustituye a las
// esperas fijas de 250-500 ms, que dependian de la carga del equipo (17/09/2026).
async function waitForFacturaIdle(timeout = 5000) {
  await new Promise(resolve => setTimeout(resolve, 0));
  await vi.waitFor(() => expect(window.__LF_FACTURA_BUSY).not.toBe(true), { timeout, interval: 10 });
  await new Promise(resolve => setTimeout(resolve, 0));
}
...
describe('Factura PDF Integration (Black Box)', () => {
  ...
  it('mueve el foco al estado de procesamiento y despues al resultado cuando oculta el area de subida', async () => {
    ...
    // Espera por condicion, no por tiempo: con la suite completa en paralelo el procesado tarda
    // mas de 100 ms y el foco seguia en el cargador (fallo intermitente, 17/09/2026).
    await vi.waitFor(() => {
