...
// Lista blanca de directorios con paginas publicas. Ver la nota extensa en
// tests/csp-inline-hash.test.js: recorrer el arbol entero con lista negra hacia que
// estos guardrails dependieran de HTML gitignorado (fixtures, dashboards locales del
// validador), fallando en local y pasando en CI. `tests/csp-inline-hash.test.js`
// incluye el centinela que avisa si se publica una seccion nueva sin anadirla aqui.
...
function walkHtmlFiles(dirPath) {
  ...
  for (const dir of PUBLIC_PAGE_DIRS) {
    const abs = path.resolve(dirPath, dir);
...
function loadPages() {
  ...
  return htmlFiles.map((filePath) => {
    ...
    return {
      ...
      ogType: normalizeWhitespace(byProperty.get('og:type') || ''),
...
function findArticleImage(html) {
  for (const object of extractJsonLdObjects(html)) {
    if (String(object['@type'] || '') !== 'Article') continue;
    if (object.image && typeof object.image === 'object') return object.image;
  }

  return null;
}

function localPathFromSiteUrl(rawUrl) {
  try {
    const url = new URL(rawUrl, BASE_URL);
    if (url.origin !== BASE_URL) return null;
    return path.join(REPO_ROOT, url.pathname.split('/').filter(Boolean).join(path.sep));
  } catch {
    return null;
  }
}
...
describe('SEO metadata guardrails', () => {
  ...
  it('keeps indexable snippets complete and bounded', () => {
    ...
    for (const page of pages) {
      ...
      if (!page.ogType) errors.push(`${page.relPath}: missing og:type`);
  ...
  it('identifies editorial guides consistently as Open Graph and JSON-LD articles', () => {
    const guides = pages.filter(
      (page) => page.indexable && page.relPath.startsWith('guias/') && page.relPath !== 'guias/index.html'
    );
    const errors = [];
    ...
    for (const page of guides) {
      if (page.ogType.toLowerCase() !== 'article') {
        errors.push(`${page.relPath}: og:type must be article`);
      }
      if (!extractJsonLdObjects(page.html).some((object) => object['@type'] === 'Article')) {
        errors.push(`${page.relPath}: missing Article JSON-LD`);
      }
    }
  ...
  it('allows large image previews on every indexable page', () => {
    const errors = [];
    ...
    for (const page of pages) {
      ...
      if (!page.robots) {
        errors.push(`${page.relPath}: missing meta robots`);
        continue;
      }

      if (!page.robots.includes('max-image-preview:large')) {
        errors.push(`${page.relPath}: robots "${page.robots}" lacks max-image-preview:large`);
      }
    }
  ...
  it('keeps guide Article.image aligned with og:image and real file dimensions', () => {
    const guides = pages.filter(
      (page) => page.indexable && page.relPath.startsWith('guias/') && page.relPath !== 'guias/index.html'
    );
    const errors = [];
    ...
    for (const page of guides) {
      const { byProperty } = extractMeta(page.html);
      const ogImage = normalizeSiteUrl(byProperty.get('og:image') || '');
      const articleImage = findArticleImage(page.html);

      if (!ogImage) {
        errors.push(`${page.relPath}: missing or off-domain og:image`);
        continue;
      }

      if (!articleImage) {
        errors.push(`${page.relPath}: missing Article.image ImageObject`);
        continue;
      }

      // La imagen del Article debe ser la del articulo, no una generica del sitio.
      const articleUrl = normalizeSiteUrl(String(articleImage.url || ''));
      if (articleUrl !== ogImage) {
        errors.push(`${page.relPath}: Article.image ${articleUrl} does not match og:image ${ogImage}`);
        continue;
      }

      const localPath = localPathFromSiteUrl(ogImage);
      if (!localPath || !fs.existsSync(localPath)) {
        errors.push(`${page.relPath}: og:image ${ogImage} has no local file`);
        continue;
      }

      const real = readPngSize(localPath);
      if (!real) continue;

      // width/height son opcionales en schema.org; si estan, no pueden mentir.
      const declaredWidth = articleImage.width;
      const declaredHeight = articleImage.height;
      const hasWidth = declaredWidth !== undefined && declaredWidth !== null;
      const hasHeight = declaredHeight !== undefined && declaredHeight !== null;

      if (hasWidth !== hasHeight) {
        errors.push(`${page.relPath}: Article.image declares only one of width/height`);
      } else if (hasWidth && (Number(declaredWidth) !== real.width || Number(declaredHeight) !== real.height)) {
        errors.push(
          `${page.relPath}: Article.image declares ${declaredWidth}x${declaredHeight} but file is ${real.width}x${real.height}`
        );
      }

      const ogWidth = Number(byProperty.get('og:image:width'));
      const ogHeight = Number(byProperty.get('og:image:height'));

      if (ogWidth !== real.width || ogHeight !== real.height) {
        errors.push(
          `${page.relPath}: og:image dimensions ${ogWidth}x${ogHeight} do not match file ${real.width}x${real.height}`
        );
      }
    }
