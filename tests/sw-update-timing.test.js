...
describe('SW deferred reload timing', () => {
  ...
  beforeEach(() => {
    // Cada test crea un coordinador que escucha en window/document. Sin retirarlos, los de tests
    // anteriores seguian reaccionando (online, focus, visibilitychange) y el resultado dependia
  ...
  // Ronda 42: la pagina arranca YA controlada por un SW mas nuevo que su HTML (el controllerchange
  // llego antes de que el coordinador existiera). El SW del arnes es 20260722-080441.
  it('marca obsoleta una pagina que arranca bajo un SW mas nuevo que su build y recarga una vez', async () => {
    window.__LF_BUILD_ID = '20260722-070000';
    window.LF.initSwUpdate({ swUrl: '/sw.js' });
    await vi.advanceTimersByTimeAsync(0);

    // Respeta la ventana de 10 s tras la carga, como cualquier recarga por SW nuevo.
    expect(reloadPage).not.toHaveBeenCalled();
    await vi.advanceTimersByTimeAsync(10_100);
  ...
  it('no vuelve a recargar si ya lo hizo para esa version aunque el HTML siga siendo antiguo', async () => {
    sessionStorage.setItem('__LF_SW_RELOADED_VERSION__:/', '20260722-080441');
    window.__LF_BUILD_ID = '20260722-070000';
    window.LF.initSwUpdate({ swUrl: '/sw.js' });
    await vi.advanceTimersByTimeAsync(10_100);
  ...
  it.each([
    ['del mismo build', '20260722-080441'],
    ['mas nueva que su SW', '20260722-090000'],
    ['sin build reconocible', 'unknown']
  ])('no marca obsoleta una pagina %s', async (_label, build) => {
    window.__LF_BUILD_ID = build;
    window.LF.initSwUpdate({ swUrl: '/sw.js' });
    await vi.advanceTimersByTimeAsync(10_100);
  ...
  it('detecta un HTML antiguo cuando el primer SW que toma el control ya es mas nuevo', async () => {
    // Visitante nuevo: el CDN entrega aun el HTML anterior y el SW que se instala es el nuevo.
    const firstController = serviceWorker.controller;
    serviceWorker.controller = null;
    window.__LF_BUILD_ID = '20260722-070000';
    window.LF.initSwUpdate({ swUrl: '/sw.js' });
  ...
  it('la primera instalacion con el HTML del mismo build no recarga', async () => {
    const firstController = serviceWorker.controller;
    serviceWorker.controller = null;
    window.__LF_BUILD_ID = '20260722-080441';
    window.LF.initSwUpdate({ swUrl: '/sw.js' });
  ...
  it('no recarga tras el controllerchange si la pagina ya es del build del SW nuevo', async () => {
    // Primera visita tras un despliegue: el HTML nuevo llega por red y despues se activa el SW
    // nuevo. Antes se recargaba a los 10 s sin necesidad (reproducido en Chrome real).
    window.__LF_BUILD_ID = '20260722-080441';
    window.LF.initSwUpdate({ swUrl: '/sw.js' });
    swHandlers.controllerchange();
    await vi.advanceTimersByTimeAsync(10_100);
  ...
  it('no recarga si el controlador activo no es mas nuevo que la pagina', async () => {
    window.__LF_BUILD_ID = '20260722-090000';
    window.LF.initSwUpdate({ swUrl: '/sw.js' });
    swHandlers.controllerchange();
    await vi.advanceTimersByTimeAsync(10_100);
  ...
  it('sigue recargando una vez tras el controllerchange si la pagina es de un build anterior', async () => {
    window.__LF_BUILD_ID = '20260722-070000';
    window.LF.initSwUpdate({ swUrl: '/sw.js' });
    swHandlers.controllerchange();
    await vi.advanceTimersByTimeAsync(10_100);
  ...
  it('si GET_VERSION no responde a tiempo al arrancar, la comprobacion se reintenta despues', async () => {
    // Revision de la ronda 42: con un timeout, la deteccion se perdia para siempre porque todos
    // los disparadores posteriores salian por `!pageIsStale`.
    let calls = 0;
    serviceWorker.controller = {
      postMessage(_message, ports) {
        calls += 1;
        const channel = ports[0].owner;
        const answer = () => channel.port1.onmessage({ data: { version: '20260722-080441' } });
        if (calls === 1) setTimeout(answer, 1_100);
