...
describe('comparador principal con módulos parciales', () => {
  ...
  it('también deja un aviso accionable si falta por completo el namespace LF', () => {
    document.body.innerHTML = `
      <span id="statusText">Introduce los datos de tu factura</span>
...
describe('comparador principal ante un microcorte de tarifas', () => {
  const loadLfApp = new Function('window', readJs('lf-app.js'));

  function createReadyLf(baseTarifasCache) {
    const noop = vi.fn();
    const calculateLocal = vi.fn(async () => {});
    const toast = vi.fn();
    const setStatus = vi.fn();
    const fetchTarifas = vi.fn(async () => false);
    const LF = {
      $: vi.fn(() => null),
      el: {},
      state: { rows: [], sort: {} },
      ...
      setStatus,
      markPending: noop,
      applyThemeClass: noop,
      updateThemeIcon: noop,
      toggleTheme: noop,
      initTooltips: noop,
      fetchTarifas,
      getInputValues: vi.fn(() => ({ consumo: 100 })),
      signatureFromValues: vi.fn(() => 'firma'),
      validateInputs: vi.fn(() => true),
      loadInputs: noop,
      saveInputs: noop,
      updateKwhHint: noop,
      updateZonaFiscalUI: noop,
      updateSolarUI: noop,
      calculateLocal,
      renderTable: noop,
      updateSortIcons: noop,
      initCSVImporter: noop,
      updateMiTarifaForm: noop,
      agregarMiTarifa: vi.fn(() => null),
      validateMiTarifa: vi.fn(() => true),
      baseTarifasCache
    };
    return { LF, calculateLocal, fetchTarifas, toast, setStatus };
  }
  ...
  it('continúa con las tarifas descargadas en esta sesión si falla el refresco', async () => {
    const sessionTarifas = [{ nombre: 'Tarifa válida de sesión' }];
    const harness = createReadyLf(sessionTarifas);
    window.LF = harness.LF;
    vi.stubGlobal('requestAnimationFrame', (callback) => callback());
    loadLfApp(window);

    window.runCalculation();
    await vi.waitFor(() => expect(harness.calculateLocal).toHaveBeenCalledTimes(1));

    expect(harness.fetchTarifas).toHaveBeenCalledWith(false, {
      silent: true,
      diagnosticReason: 'calculate'
    });
    expect(window.LF.cachedTarifas).toEqual(sessionTarifas);
    expect(harness.toast).toHaveBeenCalledWith(
      expect.stringContaining('última descarga de esta sesión'),
      'err'
    );
  });

  it('aborta si nunca hubo una descarga válida en la sesión', async () => {
    const harness = createReadyLf([]);
    window.LF = harness.LF;
    vi.stubGlobal('requestAnimationFrame', (callback) => callback());
    loadLfApp(window);

    window.runCalculation();
    await vi.waitFor(() => expect(harness.setStatus).toHaveBeenCalledWith('Error conexión', 'err'));

    expect(harness.calculateLocal).not.toHaveBeenCalled();
    expect(harness.toast).toHaveBeenCalledWith('Error cargando tarifas desde el servidor.', 'err');
  });
});
