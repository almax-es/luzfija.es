...
describe('Importación robusta CSV (Datadis, e-distribución, i-DE)', () => {
  ...
  it('Procesa el fixture Datadis nuevo tambien por la ruta solar', async () => {
    const csvContent = fs.readFileSync(path.resolve(__dirname, 'fixtures/datadis_nuevo.csv'), 'utf8');
    const file = {
      name: 'datadis.csv',
      _content: csvContent,
    ...
    expect(result.records.reduce((acc, r) => acc + r.excedente, 0)).toBeCloseTo(0.4, 6);
    expect(result.warnings.some(w => /energia_generada|columna solar/i.test(w))).toBe(false);
  });
  ...
  it('normaliza el CCH-CONS 1-23 de marzo a las horas reales de Peninsula', async () => {
    const cchConsHours = Array.from({ length: 23 }, (_, i) => i + 1);
    const canonicalHours = [1, 2, ...Array.from({ length: 21 }, (_, i) => i + 4)];
    const csvContent = [
      'CUPS;Fecha;Hora;Consumo_kWh;Método',
      ...cchConsHours.map((hour) => `ES123;29/03/2026;${hour};1,0;R`)
    ].join('\n');
    ...
    expect(records.map(r => r.hora)).toEqual(canonicalHours);
    expect(result.warnings.some(w => /CCH-CONS 1-23/i.test(w))).toBe(true);

    const totalImport = records.reduce((acc, r) => acc + r.kwh, 0);
    expect(totalImport).toBeCloseTo(23, 6);
  });

  it('normaliza el CCH-CONS 1-23 de marzo con el salto propio de Canarias', async () => {
    const cchConsHours = Array.from({ length: 23 }, (_, i) => i + 1);
    const csvContent = [
      'CUPS;Fecha;Hora;Consumo_kWh;Método',
      ...cchConsHours.map((hour) => `ES123;29/03/2026;${hour};1,0;R`)
    ...
    const file = {
      name: 'dst-marzo-canarias.csv',
      _content: csvContent,
      size: csvContent.length,
      type: 'text/csv'
    };

    const result = await window.BVSim.importFile(file, 'Canarias');

    expect(result.ok).toBe(true);
    expect(result.records.map(r => r.hora)).toEqual([
      1,
      ...Array.from({ length: 22 }, (_, i) => i + 3)
    ]);
    expect(result.records.every(r => r.periodo === 'P3')).toBe(true);
  });

  it('conserva una curva de marzo que ya declara el hueco horario real', async () => {
    const canonicalHours = [1, 2, ...Array.from({ length: 21 }, (_, i) => i + 4)];
    const csvContent = [
      'CUPS;Fecha;Hora;Consumo_kWh;Método',
      ...canonicalHours.map((hour) => `ES123;29/03/2026;${hour};1,0;R`)
    ].join('\n');
    const file = { name: 'dst-marzo-canonico.csv', _content: csvContent };

    const result = await procesarCSVConsumos(file);

    expect(result.ok).toBe(true);
    expect(result.consumosHorarios.map(r => r.hora)).toEqual(canonicalHours);
    expect(result.warnings.some(w => /CCH-CONS 1-23/i.test(w))).toBe(false);
  });

  it('reclasifica una curva importada de Peninsula a Ceuta sin conservar record.periodo', async () => {
    const csvContent = [
      'CUPS;Fecha;Hora;Consumo_kWh;Método',
      'ES123;02/06/2026;11;1,0;R',
      'ES123;02/06/2026;15;2,0;R',
      'ES123;02/06/2026;19;4,0;R',
      'ES123;02/06/2026;23;8,0;R'
    ].join('\n');
    const result = await procesarCSVConsumos({ name: 'cambio-zona.csv', _content: csvContent });

    expect(result.ok).toBe(true);
    expect(result.punta).toBe('5,00');
    expect(result.llano).toBe('10,00');
    expect(result.consumosHorarios.map(record => record.periodo)).toEqual(['P1', 'P2', 'P1', 'P2']);

    const ceuta = window.LF.reclasificarConsumosHorarios(result.consumosHorarios, 'CeutaMelilla');

    expect(ceuta.ok).toBe(true);
    expect(ceuta.punta).toBe('10,00');
    expect(ceuta.llano).toBe('5,00');
    expect(ceuta.valle).toBe('0,00');
    expect(ceuta.records.map(record => record.periodo)).toEqual(['P2', 'P1', 'P2', 'P1']);
    expect(ceuta.records).not.toBe(result.consumosHorarios);
  });
...
ES123;01/01/2024;1;1,0;R`;
  ...
  it('Validación de meses distintos (modo comparador principal vs solar)', () => {
    ...
    const records13 = [];
    const base = new Date(2024, 0, 1); // fecha civil local: '2024-01-01' seria UTC y en America caeria en diciembre
    for(let i=0; i<13; i++) {
  ...
  it('Modo solar (requireExactly12Months) rechaza un hueco mensual interno (14/08/2026)', () => {
    const { validateCsvSpanFromRecords } = window.LF.csvUtils;
    const base = new Date(2024, 0, 1); // fecha civil local: '2024-01-01' seria UTC y en America caeria en diciembre
    // 12 meses distintos pero SIN julio (indice 6): ene-jun + ago-ene siguiente.
    const indices = [0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12];
    const records = indices.map((i) =>
      ({ fecha: new Date(base.getFullYear(), base.getMonth() + i, 1) }));
    ...
    expect(res.ok).toBe(false);
    expect(res.error).toMatch(/hueco/i);
    expect(res.error).toContain('junio 2024');
    expect(res.error).toContain('agosto 2024');
  });

  it('Modo solar sigue aceptando 12 meses consecutivos normales (regresion 14/08/2026)', () => {
    const { validateCsvSpanFromRecords } = window.LF.csvUtils;
    const base = new Date(2024, 0, 1); // fecha civil local: '2024-01-01' seria UTC y en America caeria en diciembre
    const records = [];
    for (let i = 0; i < 12; i++) {
      records.push({ fecha: new Date(base.getFullYear(), base.getMonth() + i, 1) });
    }
    const res = validateCsvSpanFromRecords(records, {
      maxDays: 400,
      requireExactly12Months: true
    ...
    expect(res.ok).toBe(true);
    expect(res.monthsUsed).toHaveLength(12);
  });

  it('El comparador PRINCIPAL sigue aceptando meses no consecutivos (14/08/2026, no aplica ahi)', () => {
    // La exigencia de continuidad es exclusiva del modo solar (BV encadena meses).
    // La home acepta huecos; la semantica economica usa aparte los dias con registros.
    const { validateCsvSpanFromRecords } = window.LF.csvUtils;
    const base = new Date(2024, 0, 1); // fecha civil local: '2024-01-01' seria UTC y en America caeria en diciembre
    const indices = [0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12];
    const records = indices.map((i) =>
  ...
  it('expone por separado rango civil y dias con datos en una curva discontinua', async () => {
    const csvContent = [
      'CUPS;Fecha;Hora;Consumo_kWh;Método',
      'ES123;01/01/2026;1;1,0;R',
      'ES123;31/01/2026;1;1,0;R'
    ].join('\n');

    const result = await procesarCSVConsumos({ name: 'curva-discontinua.csv', _content: csvContent });

    expect(result.ok).toBe(true);
    expect(result.dias).toBe(2);
    expect(result.spanDays).toBe(31);
    expect(result.spanStartYmd).toBe('2026-01-01');
    expect(result.spanEndYmd).toBe('2026-01-31');
    expect(window.LF.csvHelpers.buildCoverageDiscontinuityMessage(result)).toContain('31 días de calendario');
    expect(window.LF.csvHelpers.buildCoverageDiscontinuityMessage(result)).toContain('lecturas en 2 días');
    expect(window.LF.csvHelpers.buildCoverageDiscontinuityMessage(result)).toContain('cargos diarios');
  });

  it('El hueco mensual se clasifica como rango-fechas en analitica (14/08/2026)', () => {
    const { validateCsvSpanFromRecords, csvErrorCodeForTracking } = window.LF.csvUtils;
    const base = new Date(2024, 0, 1); // fecha civil local: '2024-01-01' seria UTC y en America caeria en diciembre
    const indices = [0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12];
    const records = indices.map((i) =>
      ({ fecha: new Date(base.getFullYear(), base.getMonth() + i, 1) }));
    ...
    expect(res.ok).toBe(false);
    expect(csvErrorCodeForTracking(res.error)).toBe('rango-fechas');
  });
});
