...
describe('LF tooltips lifecycle', () => {
  ...
  it('associa el texto visible al activador solo mientras el tooltip está abierto', () => {
    const source = document.querySelector('.tooltip');
    source.setAttribute('aria-describedby', 'contexto-existente');
    source.getBoundingClientRect = () => ({ top: 20, bottom: 40, left: 20, right: 40, width: 20, height: 20 });
    window.LF.el.globalTooltip.getBoundingClientRect = () => ({ width: 120, height: 30 });

    window.LF.initTooltips();
    source.dispatchEvent(new FocusEvent('focus'));

    expect(source.getAttribute('aria-describedby')).toBe('contexto-existente globalTooltip');
    expect(window.LF.el.globalTooltip.getAttribute('aria-hidden')).toBe('false');

    source.dispatchEvent(new FocusEvent('blur'));

    expect(source.getAttribute('aria-describedby')).toBe('contexto-existente');
    expect(window.LF.el.globalTooltip.getAttribute('aria-hidden')).toBe('true');
  });
});
