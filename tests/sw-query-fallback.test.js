...
describe('Service Worker query fallback', () => {
  ...
  it('solo usa fallback cacheado de ejecutables para el build activo', () => {
    const root = path.resolve(__dirname, '..');
    const sw = fs.readFileSync(path.join(root, 'sw.js'), 'utf8');
    ...
    expect(scriptBranch[0]).toMatch(/url\.searchParams\.get\("v"\)/);
    expect(scriptBranch[0]).toMatch(/requestedBuild !== CACHE_VERSION/);
    expect(scriptBranch[0]).toMatch(/CROSS_BUILD_RECOVERY_PATHS\.has\(url\.pathname\)/);
    expect(scriptBranch[0]).toMatch(/matchHealthyCache\(cache, req\)/);
    const recoveryBlock = sw.match(/const CROSS_BUILD_RECOVERY_PATHS = new Set\(\[([\s\S]*?)\]\);/);
    expect(recoveryBlock).not.toBeNull();
    expect([...recoveryBlock[1].matchAll(/new URL\("([^"]+)"/g)].map((match) => match[1])).toEqual([
      'js/error-bootstrap.js',
      'js/lf-sw-update.js'
    ]);
  });
