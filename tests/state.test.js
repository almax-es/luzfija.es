...
global.window = {
  LF: {},
  // Mezcla deliberada: un parametro de tracking (que debe ser IGNORADO por la configuracion)
  // y una clave real de configuracion compartible (que si debe entrar).
  location: { search: '?utm_source=test&cPunta=250' },
...
describe('Gestión de Estado (lf-state.js)', () => {
  ...
  it('SERVER_PARAMS solo admite claves de configuración compartible', () => {
    expect(LF.SERVER_PARAMS).toBeDefined();

    // La clave de configuracion si entra.
    expect(LF.SERVER_PARAMS.cPunta).toBe('250');

    // El parametro de tracking NO. La presencia de SERVER_PARAMS es lo que hace a loadInputs()
    // ignorar localStorage; si `utm_source` entrara aqui, abrir un enlace de newsletter
    // mostraria los valores por defecto y al calcular se persistirian encima de la
    // configuracion guardada del usuario.
    expect(LF.SERVER_PARAMS).not.toHaveProperty('utm_source');
    expect(Object.keys(LF.SERVER_PARAMS)).toEqual(['cPunta']);
  });

  it('la whitelist de parámetros compartibles se deriva de DEFAULTS', () => {
    // Escribirla a mano la dejaria desincronizada al anadir campos nuevos al formulario.
    expect(LF.SHAREABLE_INPUT_KEYS).toEqual(Object.keys(LF.DEFAULTS));
    // Claves que jamas deben tratarse como configuracion.
    ['utm_source', 'utm_medium', 'fbclid', 'gclid', 'ref', 'debug'].forEach((k) => {
      expect(LF.SHAREABLE_INPUT_KEYS).not.toContain(k);
    });
  });
