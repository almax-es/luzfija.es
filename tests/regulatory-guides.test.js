...
describe('Guide regulatory guardrails', () => {
  it('documents the current August tax rate and the September IPC condition', () => {
    const facturaGuide = readGuideText('guias/como-leer-tu-factura-de-la-luz-paso-a-paso.html');

    expect(facturaGuide).toContain('el IPC anual de electricidad de junio fue del 6,0%');
    expect(facturaGuide).toContain('por lo que en agosto se mantiene el IVA general del 21%');
    expect(facturaGuide).toContain('El tipo de septiembre depende del IPC definitivo de julio');
