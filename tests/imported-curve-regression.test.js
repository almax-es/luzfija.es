...
describe('Regresión - Persistencia de curva horaria importada', () => {
  ...
  const pvpcCode = fs.readFileSync(path.resolve(__dirname, '../js/pvpc.js'), 'utf8');
  ...
  it('la invalidación fina del modo CSV distingue agregados, periodos y reloj', () => {
    expect(code).toMatch(/assessCsvConsumosRef\?\.\(values,\s*window\.LF\.csvConsumosRef\)/);
    expect(code).toMatch(/periodProfileMatches/);
    expect(code).toMatch(/clockProfileMatches/);
    expect(code).toMatch(/clearCsvImportState\s*\(/);
  });

  it('reclasifica P1/P2/P3 al cambiar Ceuta y conserva la curva para PVPC exacto', () => {
    expect(code).toMatch(/reclasificarConsumosHorarios\?\.\(records,\s*values\.zonaFiscal\)/);
    expect(code).toMatch(/window\.LF\.consumosHorarios\s*=\s*reclassified\.records/);
    expect(code).toMatch(/window\.LF\.csvConsumosRef\s*=\s*window\.LF\.buildCsvConsumosRef\(nextValues\)/);
    expect(pvpcCode).toMatch(/window\.LF\?\.consumosHorarios[\s\S]*window\.LF\.pvpcPeriodoCSV\s*===\s*true/);
  });

  it('avisa si el cambio de reloj obliga a retirar una curva con DST', () => {
    expect(code).toMatch(/hasDstTransitionRecords/);
    expect(code).toContain('debes reimportarlo para volver a usar los precios horarios exactos');
  });
