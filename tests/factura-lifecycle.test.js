...
describe('compatibilidad de PDF.js', () => {
  ...
  it('el reintento del bootstrap pide el vendor del worker con otra URL HTTP (17/09/2026)', () => {
    // En modo fake-worker el vendor se importa en el realm de la pagina y un import() fallido
    // queda memorizado para su URL: el fragmento del bootstrap no basta para el import interno.
    const start = bootstrapCode.indexOf('const bootstrapUrl = new URL(import.meta.url);');
    const end = bootstrapCode.indexOf('const workerModule = await import(vendorWorkerUrl.href);');
    expect(start).toBeGreaterThan(0);
    expect(end).toBeGreaterThan(start);
    const snippet = bootstrapCode.slice(start, end).replace('import.meta.url', 'metaUrl');
    const vendorUrlFor = (metaUrl) => vm.runInNewContext(`${snippet}; vendorWorkerUrl.href`, { URL, metaUrl });

    expect(vendorUrlFor('https://luzfija.es/js/pdfjs-worker-bootstrap.mjs?v=20260917-075811'))
      .toBe('https://luzfija.es/vendor/pdfjs/pdf.worker.min.mjs?v=20260917-075811');
    expect(vendorUrlFor('https://luzfija.es/js/pdfjs-worker-bootstrap.mjs?v=20260917-075811#lf-pdf-worker-retry-2'))
      .toBe('https://luzfija.es/vendor/pdfjs/pdf.worker.min.mjs?v=20260917-075811&lf_retry=2');
    // Un fragmento ajeno no se traduce en query ni viaja al vendor.
    expect(vendorUrlFor('https://luzfija.es/js/pdfjs-worker-bootstrap.mjs?v=20260917-075811#otra-cosa'))
      .toBe('https://luzfija.es/vendor/pdfjs/pdf.worker.min.mjs?v=20260917-075811');
  });
