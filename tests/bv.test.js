...
describe('Simulador Solar Mensual (bv-sim-monthly.js)', () => {
  ...
  it('bucketizeByMonth respeta los periodos Datadis mensuales en Ceuta/Melilla', () => {
    const records = [
      { fecha: new Date(2025, 1, 1), hora: 1, kwh: 100, excedente: 0, periodo: 'P3' },
      { fecha: new Date(2025, 1, 1), hora: 11, kwh: 50, excedente: 0, periodo: 'P2' },
      { fecha: new Date(2025, 1, 1), hora: 12, kwh: 30, excedente: 0, periodo: 'P1' }
    ];

    const [feb] = window.BVSim.bucketizeByMonth(records, 'CeutaMelilla', { isDatadisMonthly: true });

    expect(feb.importByPeriod).toEqual({ P1: 30, P2: 50, P3: 100 });
  });
  ...
  it('calcMonthForTarifa: marca no calculable una tarifa que necesita SSAA si falta el mes historico', () => {
    const tarifaSimple = {
      nombre: 'Solar sin SSAA historico',
      p1: 0.1, p2: 0.1,
      cPunta: 0.1, cLlano: 0.1, cValle: 0.1,
      incluyeServiciosAjuste: false,
      fv: { exc: 0, bv: false }
    ...
    expect(row.ssaaUnavailableReason).toBe('historical-month-unavailable');
    expect(Number.isNaN(row.totalPagar)).toBe(true);

    const sim = window.BVSim.simulateForTarifaDemo({
      months: [mockMonth],
      tarifa: tarifaSimple,
      potenciaP1: 0,
      potenciaP2: 0,
    ...
    expect(sim.dataUnavailable).toBe(true);
    expect(sim.dataUnavailableReason).toMatch(/SSAA no disponible/);
    expect(Number.isNaN(sim.totals.pagado)).toBe(true);
  });
