...
const engineCode = fs.readFileSync(path.resolve(__dirname, '../js/pvpc-stats-engine.js'), 'utf8');
...
const loadPvpcStatsEngine = new Function('window', engineCode);
...
describe('Observatorio: el selector de anyo no se rompe al cruzar el anyo', () => {
  ...
  it('applyStateToControls puebla el selector ANTES de asignarle el valor', () => {
    ...
    const posPoblar = cuerpo[0].indexOf('populateYearSelector(els.year, new Date(), state)');
...
describe('Observatorio: el anyo vigente es el del dataset, no el del navegador (ronda 41)', () => {
  let originalTz;
  ...
  const inBrowser = (tz, now) => {
    process.env.TZ = tz;
    vi.setSystemTime(new Date(now));
  };

  it('Tokio el 31/12 por la tarde en Espanha no ofrece 2027 ni lo acepta por URL', () => {
    const { getAvailableYearsDesc, normalizeSelectedYears, parseParams } = window.__LF_PvpcStatsUiHelpers;
    inBrowser('Asia/Tokyo', '2026-12-31T16:00:00Z');

    expect(getAvailableYearsDesc(new Date())[0]).toBe(2026);
    expect(normalizeSelectedYears('2027', '2027')).toEqual([2026, 2025, 2024]);
    history.replaceState(null, '', '/estadisticas/');
    expect(parseParams().year).toBe('2026');
    history.replaceState(null, '', '/estadisticas/?year=2027');
    expect(parseParams().year).toBe('2026');
  });

  it('Bogota con el 01/01 ya empezado en Espanha ofrece y acepta 2027', () => {
    const { getAvailableYearsDesc, normalizeSelectedYears, parseParams } = window.__LF_PvpcStatsUiHelpers;
    inBrowser('America/Bogota', '2027-01-01T00:30:00Z');

    expect(getAvailableYearsDesc(new Date())[0]).toBe(2027);
    expect(normalizeSelectedYears('9999', '')).toEqual([2027, 2026, 2025]);
    history.replaceState(null, '', '/estadisticas/?year=2027');
    expect(parseParams().year).toBe('2027');
  });

  it('el dataset canario cambia de anyo con su propia hora', () => {
    const { getAvailableYearsDesc, parseParams } = window.__LF_PvpcStatsUiHelpers;
    inBrowser('Europe/Madrid', '2026-12-31T23:30:00Z');
  ...
  it('ningun consumidor vuelve a leer el anyo del reloj del navegador', () => {
    // Arranque, etiqueta de "anyo en curso" y comparativa no tienen test de comportamiento
    // propio: la unica lectura permitida es el fallback de getDatasetCurrentYear.
    expect(uiCode.match(/getFullYear\(\)/g)).toHaveLength(1);
    expect(uiCode).toContain('return year ? Number(year) : now.getFullYear();');
    expect(uiCode).not.toMatch(/getMonth\(\)/);
  });
});

describe('Observatorio: el anyo sigue al dataset cuando cambian zona o tipo (ronda 41, C41-02)', () => {
  let originalTz;
  ...
  beforeEach(() => {
    ...
    Object.defineProperty(document, 'readyState', { configurable: true, get: () => 'loading' });
    loadCsvUtils(window);
    // Motor REAL: la UI delega la zona en PVPC_STATS.getDatasetTimeZone y los tests anteriores
    // solo cubrian la zona de reserva.
    loadPvpcStatsEngine(window);
    loadPvpcStatsCsv(window);
    loadPvpcStatsUi(window);
    // 00:30 del 01/01/2027 en Madrid; en Canarias aun son las 23:30 del 31/12/2026.
  ...
  it('usa la zona que declara el motor para cada tipo y zona', () => {
    ...
    const spy = vi.spyOn(window.PVPC_STATS, 'getDatasetTimeZone').mockReturnValue('Pacific/Honolulu');
    expect(getDatasetCurrentYear({ geo: '8741', type: 'pvpc' })).toBe(2026);
    expect(spy).toHaveBeenCalledWith({ geoId: '8741', type: 'pvpc' });
  });
  ...
  it('al pasar a un dataset que aun no ha empezado el anyo, baja el anyo y los comparados', () => {
    const { alignStateToDataset } = window.__LF_PvpcStatsUiHelpers;
    const canarias = alignStateToDataset({ type: 'pvpc', geo: '8742', year: '2027', compareYears: [2027, 2026, 2025] });
    expect(canarias.year).toBe('2026');
    expect(canarias.compareYears).toEqual([2026, 2025]);
    ...
    expect(pasado.year).toBe('2024');
    expect(pasado.compareYears).toEqual([2024, 2023]);
  });

  it('los cambios de zona y de tipo ajustan el anyo y rehacen el selector antes de recargar', () => {
    const cuerpo = uiCode.match(/function attachControlHandlers\([\s\S]*?\n  \}/);
    expect(cuerpo).not.toBeNull();
    const handler = cuerpo[0].match(/const onDatasetChange = \(\) => \{[\s\S]*?\n    \};/);
    expect(handler).not.toBeNull();
    const pasos = ['alignStateToDataset(state)', 'populateYearSelector(els.year, new Date(), state)', 'els.year.value = state.year', 'onChange()']
      .map((paso) => handler[0].indexOf(paso));
    expect(pasos.every((pos) => pos > -1)).toBe(true);
    expect(pasos).toEqual([...pasos].sort((a, b) => a - b));
    expect(cuerpo[0]).toContain("state.type = els.type.value; onDatasetChange();");
    expect(cuerpo[0]).toContain("state.geo = els.geo.value; onDatasetChange();");
  });

  it('un chip de comparacion obsoleto no reintroduce un anyo que el dataset aun no ha empezado', () => {
    // Reproducido en Chrome: tras pasar a Canarias, pulsar el chip "2027" del render anterior
    // durante la recarga dejaba compareYears=2027 en la URL y una serie 2027 vacia.
    const inicio = uiCode.indexOf('// Comparativa años (mensual)');
    const chips = uiCode.indexOf('buildCompareYearChips(allYears, state.compareYears, toggleYear);', inicio);
    expect(inicio).toBeGreaterThan(-1);
    const tramo = uiCode.slice(inicio, chips);
    const alinear = tramo.indexOf('alignStateToDataset(state);');
    const reescribir = tramo.indexOf("if (state.compareYears.join(',') !== comparedBefore) writeParams(state, { replace: true });");
    expect(alinear).toBeGreaterThan(-1);
    expect(reescribir).toBeGreaterThan(alinear);
    const toggle = tramo.indexOf('const toggleYear = async (y) => {');
    const guarda = tramo.indexOf('if (y > getDatasetCurrentYear(state)) return;');
    const copia = tramo.indexOf('const selected = [...state.compareYears];');
    expect(toggle).toBeGreaterThan(-1);
    expect(guarda).toBeGreaterThan(toggle);
    expect(copia).toBeGreaterThan(guarda);
  });
});
