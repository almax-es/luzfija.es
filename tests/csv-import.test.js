...
01/04/2026;2;0;2,0`;
  ...
  describe('procesarXLSXConsumos', () => {
    ...
    it('Debe extraer hora 25 en matriz si existe y es válida, ignorándola si no', async () => {
      const header = ['Fecha', 'H01', 'H02', 'H03', 'H04'];
      for(let i=5; i<=25; i++) header.push(`H${String(i).padStart(2,'0')}`);
      
      const rowNormal = ['01/01/2024'];
      for(let i=1; i<=24; i++) rowNormal.push(1);
      rowNormal.push(null); // H25 vacía

      const row25 = ['27/10/2024'];
      for(let i=1; i<=25; i++) row25.push(1); // H25 con valor 1

      const mockData = [header, rowNormal, row25];
      ...
      const file = { name: 'matriz25.xlsx' };
      const result = await procesarXLSXConsumos(file);

      expect(result.dias).toBe(2);
      // Día normal (24 kWh) + Día 25h (25 kWh) = 49 kWh
      expect(result.totalKwh.replace(',', '.')).toBe('49.00');
    });
