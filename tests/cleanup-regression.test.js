...
describe('Cleanup regressions', () => {
  ...
  it('lf-sw-update.js no fuerza reload incondicional dentro de controllerchange del service worker', () => {
    ...
    // Cuerpo completo de la funcion (por llaves), no una ventana fija de caracteres: un guard
    // nuevo dentro de la funcion no debe romper este contrato.
    const bodyStart = code.indexOf('{', reloadIdx);
    let depth = 0;
    let bodyEnd = bodyStart;
    for (let i = bodyStart; i < code.length; i++) {
      if (code[i] === '{') depth++;
      if (code[i] === '}' && --depth === 0) { bodyEnd = i; break; }
    }
    const reloadBlock = code.slice(reloadIdx, bodyEnd + 1);
  ...
  it('los valores del bono social usan el color del tema activo', () => {
    const css = fs.readFileSync(path.resolve(__dirname, '../styles.css'), 'utf8');
    const rule = css.match(/#bonoSocialFields\s+\.radio-label\s*>\s*span:first-of-type\s*\{([^}]*)\}/);
    expect(rule).not.toBeNull();
    expect(rule[1]).toMatch(/color:\s*var\(--text\)/);
    expect(rule[1]).not.toMatch(/color:\s*rgba\(\s*255\s*,\s*255\s*,\s*255/);
  });

  it('los avisos de la home mantienen contraste suficiente en modo claro', () => {
    const css = fs.readFileSync(path.resolve(__dirname, '../styles.css'), 'utf8');
    expect(css).toMatch(/html\.light-mode\s+\.alert\s*\{[^}]*color:\s*#991B1B/);
    expect(css).toMatch(/html\.light-mode\s+\.alert\s+b\s*\{[^}]*color:\s*#991B1B/);
    expect(css).toMatch(/html\.light-mode\s+\.pvpc-warning\s*\{[^}]*color:\s*#92400E/);
  });
});
