...
describe('PVPC_STATS: el anyo y el mes vigentes son los del dataset, no los del navegador (ronda 41)', () => {
  // El fichero fija America/Los_Angeles arriba; cada caso cambia la zona del "navegador" y la
  // restaura. Node aplica process.env.TZ en caliente.
  async function requestedMonths({ tz, now, geoId, year }) {
    const originalFetch = global.fetch;
    const originalTz = process.env.TZ;
    const requested = [];
    global.fetch = async (url) => {
      const match = String(url).match(/\/(\d{4}-\d{2})\.json$/);
    ...
    try {
      window.PVPC_STATS.manifestCache.clear();
      await window.PVPC_STATS.buildYearData(geoId, year, 'pvpc', `ronda41-${tz}-${now}-${geoId}-${year}`);
  ...
  const monthsOf = (year, to) => Array.from({ length: to }, (_, i) => `${year}-${String(i + 1).padStart(2, '0')}`);

  it('navegador en Tokio el 31/12 por la tarde en Espanha: sigue siendo 2026, no pide 2027', async () => {
    const now = '2026-12-31T16:00:00Z';
    expect(await requestedMonths({ tz: 'Asia/Tokyo', now, geoId: 8741, year: 2026 })).toEqual(monthsOf(2026, 12));
    expect(await requestedMonths({ tz: 'Asia/Tokyo', now, geoId: 8741, year: 2027 })).toEqual([]);
  });

  it('navegador en Tokio el ultimo dia del mes: no pide todavia el mes siguiente', async () => {
    expect(await requestedMonths({ tz: 'Asia/Tokyo', now: '2026-09-30T17:00:00Z', geoId: 8741, year: 2026 }))
      .toEqual(monthsOf(2026, 9));
  });

  it('navegador en Bogota con el mes y el anyo ya empezados en Espanha: los pide', async () => {
    expect(await requestedMonths({ tz: 'America/Bogota', now: '2026-10-31T23:30:00Z', geoId: 8741, year: 2026 }))
      .toEqual(monthsOf(2026, 11));
    expect(await requestedMonths({ tz: 'America/Bogota', now: '2027-01-01T00:30:00Z', geoId: 8741, year: 2027 }))
      .toEqual(['2027-01']);
  });

  it('un anyo que aun no ha empezado en el dataset no queda cacheado vacio (C41-02)', async () => {
    const originalFetch = global.fetch;
    const originalTz = process.env.TZ;
    const requested = [];
    global.fetch = async (url) => {
    ...
    try {
      window.PVPC_STATS.cache.clear();
      window.PVPC_STATS.manifestCache.clear();
      // 00:30 en Madrid, 23:30 en Canarias: 2027 aun no ha empezado para el dataset canario.
      vi.setSystemTime(new Date('2026-12-31T23:30:00Z'));
      const early = await window.PVPC_STATS.loadYearData(8742, 2027, 'pvpc');
      expect(early.meta.daysCount).toBe(0);
      expect(requested).toEqual([]);
      expect(window.PVPC_STATS.cache.has('pvpc-8742-2027')).toBe(false);

      // Una hora despues ya es 2027 en Canarias: la misma sesion tiene que pedir enero.
      vi.setSystemTime(new Date('2027-01-01T00:30:00Z'));
      await window.PVPC_STATS.loadYearData(8742, 2027, 'pvpc');
      expect(requested).toEqual(['2027-01']);
    } finally {
      vi.useRealTimers();
      process.env.TZ = originalTz;
      global.fetch = originalFetch;
  ...
  it('Canarias usa su propia hora: a las 00:30 de Madrid del 01/01 aun no es 2027 en el dataset canario', async () => {
    const now = '2026-12-31T23:30:00Z';
    expect(await requestedMonths({ tz: 'Europe/Madrid', now, geoId: 8742, year: 2027 })).toEqual([]);
    expect(await requestedMonths({ tz: 'Europe/Madrid', now, geoId: 8741, year: 2027 })).toEqual(['2027-01']);
  });
});
