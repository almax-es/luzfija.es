...
describe('Desglose de Factura (desglose-factura.js)', () => {
  ...
  it('muestra 8,08 € para el P1 de Spock sin trasladar un céntimo a otro periodo', () => {
    Desglose.init();
    const datos = {
      nombreTarifa: 'Spock Tarifa Luz 24 Horas',
      potenciaP1: 4, potenciaP2: 4, dias: 30,
      precioP1: 0.09863, precioP2: 0.09863,
      consumoPunta: 85, consumoLlano: 0, consumoValle: 0,
      precioPunta: 0.095, precioLlano: 0.095, precioValle: 0.095,
      zonaFiscal: 'Península', fechaYmd: '2026-09-30', solarOn: false
    };
    const res = Desglose.calcularDesglose(datos);
    Desglose.renderizar(res, datos);

    expect(res.consBase).toBe(8.08);
    const text = Desglose.modal.querySelector('.desglose-body').textContent;
    expect(text).toContain('8,08');
  ...
  it('recomienda el Simulador Solar en el desglose solo con autoconsumo', () => {
    Desglose.init();
    const datosBase = {
      nombreTarifa: 'Tarifa de prueba',
      potenciaP1: 4, potenciaP2: 4, dias: 30,
      precioP1: 0.1, precioP2: 0.1,
      consumoPunta: 100, precioPunta: 0.1,
      zonaFiscal: 'Península'
    };

    const render = (solarOn) => {
      const datos = { ...datosBase, solarOn };
      Desglose.renderizar(Desglose.calcularDesglose(datos), datos);
      return Desglose.modal.querySelector('.desglose-solar-simulator');
    };

    const avisoSolar = render(true);
    expect(avisoSolar).not.toBeNull();
    expect(avisoSolar.querySelector('a')?.getAttribute('href')).toBe('/comparador-tarifas-solares.html');
    expect(avisoSolar.textContent).toContain('este ranking es orientativo');
    expect(avisoSolar.textContent).toContain('arrastra la batería virtual');
