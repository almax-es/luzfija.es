...
| Factura PDF/QR CNMC y censo R2 | Formato de URL QR, campos, codigos de contrato/factura, cambios en modelos y altas/cambios de comercializadoras | Censo mensual automatizado; solo altas pequeñas y puramente aditivas se publican solas. Revisar manualmente bajas, renombrados, contactos, lotes grandes o cambios en `_meta.count/sourceRows/duplicateCodes/invalidWebsiteCodes/inactiveCodes`; regenerar antes de publicar cambios del extractor y cuando fallen facturas reales | Resolucion CNMC publicada en BOE, censo publico CNMC y facturas reales anonimizadas | `js/factura-parsers.js`, `js/factura.js`, `data/cnmc-commercializers.json`, `scripts/sync-cnmc-commercializers.mjs`, `scripts/classify-cnmc-commercializers-update.mjs`, `.github/workflows/cnmc-commercializers.yml`, tests de factura y censo |
