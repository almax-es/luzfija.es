...
`error-context/fetch-terminal/tarifas/<recovered|failed>/<motivo>/a2/<build>`:
`recovered` si el reintento funciona y `failed` si ambos intentos fallan. Los
...
Ademas, cada fallo de script o `init-incompleto` deja en memoria una solicitud
cerrada `{ app, dependency, build }`. `lf-sw-update.js` la consume, fuerza
...
para repetir una descarga incompleta. Esta cola es funcional, no telemetria, y
no contiene URL, mensaje, stack ni datos del usuario.
