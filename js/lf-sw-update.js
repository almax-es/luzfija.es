...
(function () {
  ...
  window.LF = window.LF || {};
  const BUILD_ID_PATTERN = /^\d{8}-\d{6}$/;
  ...
  window.LF.initSwUpdate = function initSwUpdate(opts) {
    ...
    // Una pagina puede arrancar YA controlada por un SW mas nuevo que su HTML: otra pestaña activo
    // el despliegue mientras esta cargaba y el controllerchange llego antes de que existiera su
    // listener. Entonces hasSeenController nace en true y la pagina nunca se marcaba obsoleta,
    // aunque sus cargas tardias ya reciben bytes del build nuevo (ronda 42, 17/09/2026).
    // El build de la pagina es el `?v=` con el que su HTML pidio tracking.js, no el contenido
    // recibido. Solo cuenta un SW MAS NUEVO: una pagina mas nueva que su SW se resuelve con la
    // actualizacion normal. Los builds son AAAAMMDD-HHMMSS y se comparan como texto.
    // Si GET_VERSION no responde, la comprobacion sigue pendiente y la reintentan los disparadores
    // de visible/focus/online/intervalo; sin eso, un timeout la perdia para siempre.
    const pageBuildAtInit = typeof window.__LF_BUILD_ID === 'string' ? window.__LF_BUILD_ID.trim() : '';
    let pageBuildCheckPending = supportsServiceWorker && BUILD_ID_PATTERN.test(pageBuildAtInit);
    let pageBuildCheckInFlight = false;

    function checkPageBuildAgainstSw(reason) {
      if (!pageBuildCheckPending || pageBuildCheckInFlight || pageIsStale) return;
      const controller = navigator.serviceWorker.controller;
      // Sin controlador todavia no hay nada que comparar: la comprobacion sigue pendiente y la
      // repite el primer controllerchange. Un visitante nuevo puede recibir un HTML antiguo del CDN
      // mientras el SW que se instala ya es el nuevo (visto en produccion el 17/09/2026).
      if (!controller) return;
      pageBuildCheckInFlight = true;
      getSwVersion(controller).then(function (version) {
        pageBuildCheckInFlight = false;
        if (!version) {
          dbg('[SW] Page build check pending: SW version unreadable on ' + reason + '.');
          return;
        }
        pageBuildCheckPending = false;
        if (!BUILD_ID_PATTERN.test(version) || version <= pageBuildAtInit || pageIsStale) return;
        dbg('[SW] Page build ' + pageBuildAtInit + ' is older than active SW ' + version + '.');
        pageIsStale = true;
        tryReloadOnStale('build-mismatch');
      });
    }
    ...
    async function tryReloadOnStale(reason) {
      ...
      try {
        ...
        // La pagina ya es de ese build (o de uno posterior): no hay nada que refrescar. Pasaba en la
        // primera visita tras cada despliegue: el HTML nuevo llega por red, despues se activa el SW
        // nuevo y el controllerchange recargaba a los 10 s sin necesidad (ronda 42, 17/09/2026).
        // Tambien cubre un controlador que no sea mas nuevo que el documento.
        if (BUILD_ID_PATTERN.test(pageBuildAtInit) && BUILD_ID_PATTERN.test(version) && version <= pageBuildAtInit) {
          pageIsStale = false;
          clearStaleReloadTimer();
          dbg('[SW] Page already on build ' + pageBuildAtInit + ' (SW ' + version + '). No reload needed.');
          return;
        }
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible') ric(() => {
        ...
        checkPageBuildAgainstSw('visible');
    ...
    window.addEventListener('focus', () => ric(() => {
      ...
      checkPageBuildAgainstSw('focus');
      tryReloadOnStale('focus');
    }));
    window.addEventListener('online', () => {
      requestSwUpdate('online');
      checkPageBuildAgainstSw('online');
    ...
    setInterval(() => {
      ...
      ric(() => {
        ...
        checkPageBuildAgainstSw('interval');
    ...
    if (supportsServiceWorker) navigator.serviceWorker.addEventListener('controllerchange', function () {
      ...
      if (!hasSeenController) {
        ...
        // Primera instalacion no implica pagina actual: si el HTML es de un build anterior al SW
        // que acaba de tomar el control, se marca obsoleta como en cualquier otra comprobacion.
        checkPageBuildAgainstSw('first-install');
        return;
      }
