...
const PVPC_STATS = {
    ...
    async buildYearData(geoId, year, type = 'pvpc', cacheKey = `${type}-${geoId}-${year}`) {
        ...
        // El anyo y el mes vigentes salen del MISMO dia: con getFullYear()/getMonth() un
        // navegador en Tokio pedia el 31/12 solo enero del anyo siguiente, y uno en Bogota
        // no pedia el mes nuevo hasta horas despues de empezar en Espanha (ronda 41).
        const now = new Date();
        const todayParts = /^(\d{4})-(\d{2})-\d{2}$/.exec(todayLocal || '');
        const currentYear = todayParts ? Number(todayParts[1]) : now.getFullYear();
        const currentMonth = todayParts ? Number(todayParts[2]) : now.getMonth() + 1;
        ...
        // Un anyo que aun no ha empezado en la zona del dataset no pide ningun mes: no es un anyo
        // "completo y vacio". Cachearlo lo dejaba vacio toda la sesion aunque el anyo empezara
        // minutos despues (Canarias va una hora por detras; ronda 41, C41-02).
        if (monthLabels.length > 0 && !yearData.meta.partial && yearData.meta.provisionalDays.length === 0) {
