...
// En modo fake-worker este import corre en el realm de la pagina, y Chromium memoriza un import()
// fallido para esa URL: tras un corte de red, repetir la misma URL HTTP fallaba el resto de la
// sesion. Cada reintento de factura.js (#lf-pdf-worker-retry-N) pide el vendor con otra query;
// `v` se conserva intacto para el guard de build del service worker.
const retryMatch = /^#lf-pdf-worker-retry-(\d+)$/.exec(bootstrapUrl.hash);
if (retryMatch) vendorWorkerUrl.searchParams.set('lf_retry', retryMatch[1]);
