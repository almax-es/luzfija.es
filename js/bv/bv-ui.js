...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function saveManualData() {
    ...
    try {
      // La zona con la que se repartio el consumo viaja CON los datos: sin ella, tras una
      // recarga no habria forma de saber que la tabla es de otra zona horaria y se calcularia
      // con periodos de una y fiscalidad de otra. Los registros del CSV no se persisten (son
      // grandes y privados), solo la procedencia. loadManualData lee los meses por indice
      // numerico 0..11, asi que esta clave extra no le interfiere.
      // OJO AL ORDEN: quien llame a saveManualData debe haber anotado ya la zona en
      // manualGridImportState, o aqui se persiste la anterior.
      const payload = manualGridImportState.zonaFiscal
        ? Object.assign({}, data, { zonaOrigen: manualGridImportState.zonaFiscal })
        : data;
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (event) => {
        try {
          ...
          // Recargar datos en la interfaz. loadManualData ya limpia la procedencia y
          // restaura la zona de origen si el respaldo la traia (los respaldos se exportan
          // desde bv_manual_data_v2, asi que la incluyen): limpiarla aqui de nuevo la
          // borraria justo despues de recuperarla.
          loadManualData();
          clearHourlyTraceState();
          updateManualTotals();
          updateMesInicioSelectorFromGrid();
  ...
  if (zonaFiscalInput) {
    zonaFiscalInput.addEventListener('change', () => {
      ...
      if (changesSchedule && !manualGridImportState.dirty && manualGridImportState.result) {
        ...
        // La curva horaria se re-apunta a la zona nueva junto con el grid; si no, las
        // indexadas caerian al precio de referencia pidiendo reimportar justo despues de
        // haber recalculado.
  ...
  simulateButton.addEventListener('click', async () => {
    ...
    try {
      ...
      const rankedResults = rankableResults.sort((a, b) => {
        const diffPagado = a.totals.pagado - b.totals.pagado;
        if (Math.abs(diffPagado) < 0.01) {
          const bvA = a.totals.bvFinal;
          const bvB = b.totals.bvFinal;
          return bvB - bvA;
        }
        return diffPagado;
      });
...
💡 Disponible mes siguiente`
      ...
      const invalidTariffNames = invalidResults.map((result) => escapeHtml(result?.tarifa?.nombre || 'Tarifa sin nombre'));
      const invalidRankingMsg = invalidTariffNames.length > 0
        ? `<br><br><strong>⚠️ ${invalidTariffNames.length === 1 ? 'Tarifa excluida' : 'Tarifas excluidas'}:</strong> ${invalidTariffNames.join(', ')} ${invalidTariffNames.length === 1 ? 'tiene' : 'tienen'} totales no válidos; no se muestra ningún importe para evitar resultados engañosos. Revisa sus datos y vuelve a calcular.`
        : '';
