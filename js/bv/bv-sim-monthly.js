...
window.BVSim.bucketizeByMonth = function (records, zona = 'peninsula', opts = {}) {
  ...
  (records || []).forEach((record) => {
    ...
    if (Number.isFinite(importKwh)) {
      ...
      let periodo;
      if (esCeutaMelilla && !isDatadisMonthly && Number.isFinite(hora)) {
...
window.BVSim.INDEXED_SURPLUS_REFERENCE_PRICE = window.LF_CONFIG?.INDEXED_SURPLUS_REFERENCE_PRICE ?? 0.02;
...
window.BVSim.calcMonthForTarifa = function ({
  ...
  const roundMoneyProducts = (window.LF_CONFIG && typeof window.LF_CONFIG.roundMoneyProducts === 'function')
    ? window.LF_CONFIG.roundMoneyProducts
    : (products) => round2((products || []).reduce((sum, factors) => sum + factors.reduce((prod, factor) => prod * Number(factor), 1), 0));
  ...
  // Potencia
  const pot = roundMoneyProducts([[potenciaP1, dias, tarifa.p1], [potenciaP2, dias, tarifa.p2]]);

  // Consumo (Energía)
  const consBaseEur = roundMoneyProducts([
  ...
  if (ssaa.aplica && ssaa.available === false) {
    return {
      key: month.key,
      dias,
      pot,
      ...
      dataUnavailableReason: `SSAA no disponible para ${month.key}: no se puede valorar esta tarifa sin omitir un coste regulado.`,
      ssaaUnavailableReason: ssaa.reason || 'dataset-unavailable',
      bvSaldoPrev: bvPrev,
      bvSaldoFin: bvPrev,
      credit1: 0,
      credit2: 0,
      totalPagar: Number.NaN,
      totalReal: Number.NaN
    };
  }
  ...
  if (esIndexada) {
    ...
    if (hasIndexedHourlyValue) {
      ...
      creditoPotencial = roundMoneyProducts([[exKwh, precioExc]]);
  ...
  // Cuota BV prorrateada por mes natural (dias/daysInMonth): aquí se simula un mes
  // concreto del CSV, así que el prorrateo exacto es posible. DELIBERADO: difiere de
  // la home (lf-calc.js), que anualiza ×12/365 porque su periodo es genérico (N días
  // sin mes asociado, misma convención que el alquiler de contador). No es un bug.
  ...
  if (esCompParcial) {
    ...
    peajesTotal = roundMoneyProducts([
  ...
  const fiscalDateYmd = (() => {
    ...
    if (!m) {
      console.warn('[BVSim] month.key inesperado para fecha fiscal:', key);
      return undefined;
    }
  ...
  return {
    ...
    indexedMissingKwh: Number.isFinite(Number(month?.indexedMissingKwh)) ? Number(month.indexedMissingKwh) : 0,
    indexedPricedHours: Number.isFinite(Number(month?.indexedPricedHours)) ? Number(month.indexedPricedHours) : 0,
    indexedSurplusWarning: month?.indexedSurplusWarning || '',
    credit1,
...
window.BVSim.simulateForTarifaDemo = function ({
  ...
  const unavailableRow = rows.find((row) => row?.dataUnavailable);
  return {
    ok: true,
    tarifa,
    rows,
    dataUnavailable: Boolean(unavailableRow),
    dataUnavailableReason: unavailableRow?.dataUnavailableReason || null,
    totals: {
...
window.BVSim.loadTarifasBV = async function () {
  try {
    ...
    // Todo o nada, igual que el comparador (ver lf-cache.js): si una sola fila es
    // inutilizable se rechaza el dataset entero en vez de dar un ranking parcial sin
    // avisar. Un ranking incompleto puede dejar como "más barata" una tarifa que solo
    // parece ganadora porque otra mejor desapareció en silencio. Fail-closed: si la
    // dependencia que garantiza esta integridad no está disponible (p. ej. un error de
    // orden de scripts), no se degrada a validar sin ella.
    const esTarifaUtilizable = window.LF?.esTarifaUtilizable;
    if (typeof esTarifaUtilizable !== 'function') {
      const fn = window.LF?.lfDbg || window.lfDbg;
      if (typeof fn === 'function') fn('[BVSim] window.LF.esTarifaUtilizable no disponible; ¿lf-utils.js cargó antes que bv-sim-monthly.js?');
      return {
        ok: false,
        error: 'No se pudo validar tarifas.json; no se muestra el ranking para evitar resultados engañosos.'
      };
    }
    if (tarifas.length > 0 && !tarifas.every(esTarifaUtilizable)) {
      return {
        ok: false,
        error: 'tarifas.json contiene datos inválidos; no se muestra el ranking para evitar resultados engañosos.'
      };
    }
