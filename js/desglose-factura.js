...
(function() {
  ...
  Object.assign(Desglose, {
    ...
    _openSeq: 0,
    _lastFocusedEl: null,
    _scrollLockToken: null,
    _scrollFallbackState: null,
      ...
      this.modal.innerHTML = `
        ...
        <div class="desglose-content">
          <div class="desglose-info">
            ...
            <div class="desglose-promo" style="display:none;"></div>
          </div>
    ...
    abrir(datos) {
      ...
      if (!this._scrollLockToken && !this._scrollFallbackState) {
        const sharedScrollLock = window.LF?.modalScrollLock;
        if (sharedScrollLock && typeof sharedScrollLock.lock === 'function') {
          this._scrollLockToken = sharedScrollLock.lock('desglose-factura');
        } else {
          this._scrollFallbackState = {
            scrollTop: Number(document.body?.scrollTop) || 0,
    ...
    cerrar() {
      ...
      this.modal.setAttribute('aria-hidden', 'true');
      const sharedScrollLock = window.LF?.modalScrollLock;
      if (this._scrollLockToken && sharedScrollLock && typeof sharedScrollLock.unlock === 'function') {
        sharedScrollLock.unlock(this._scrollLockToken);
      } else if (this._scrollFallbackState) {
        document.documentElement.style.overflow = this._scrollFallbackState.htmlOverflow;
        document.body.style.overflow = this._scrollFallbackState.bodyOverflow;
        document.body.scrollTop = this._scrollFallbackState.scrollTop;
      }
      this._scrollLockToken = null;
      this._scrollFallbackState = null;
