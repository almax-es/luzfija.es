...
function remapLegacyGoatPayload(payload, kind) {
  ...
  return {
    // Mismo esquema segmentado que js/tracking.js (legacyNoiseEventPath): el tipo
    // y el build tienen que ir en el PATH, no solo en el title, porque
    // GoatCounter solo sustituye el title de una ruta tras 10+ repeticiones y
    // ambos tipos de ruido quedaban sumados en una unica fila indistinguible.
    path: 'error-legacy-filtrado/' + legacyNoiseSegment(kind, 'legacy') + '/' +
      (/^\d{8}-\d{6}$/.test(buildId) ? buildId : 'desconocido'),
    title: parts.join(' | ').substring(0, 150),
    event: true
  };
}

// Equivalente reducido de eventSegment() de tracking.js: config.js se carga antes
// y no comparte sus helpers, pero el path debe salir con la misma forma.
function legacyNoiseSegment(value, fallback) {
