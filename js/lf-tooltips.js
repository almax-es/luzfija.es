...
(function() {
  ...
  function addTooltipDescription(target) {
    const tooltipId = el.globalTooltip?.id;
    if (!tooltipId || target.__LF_TOOLTIP_DESCRIPTION_ADDED) return;

    const descriptions = (target.getAttribute('aria-describedby') || '').split(/\s+/).filter(Boolean);
    if (descriptions.includes(tooltipId)) return;

    descriptions.push(tooltipId);
    target.setAttribute('aria-describedby', descriptions.join(' '));
    target.__LF_TOOLTIP_DESCRIPTION_ADDED = true;
  }

  function removeTooltipDescription(target) {
    if (!target?.__LF_TOOLTIP_DESCRIPTION_ADDED) return;
    ...
    delete target.__LF_TOOLTIP_DESCRIPTION_ADDED;
  }
  ...
  function positionTooltip(target) {
    ...
    tooltipRaf = requestAnimationFrame(() => {
      ...
      if (tip) addTooltipDescription(target);
      else removeTooltipDescription(target);
  ...
  function hideTooltip(force = false) {
    ...
    removeTooltipDescription(activeTooltip);
  ...
  function openTooltip(target) {
    ...
    if (activeTooltip && activeTooltip !== target) removeTooltipDescription(activeTooltip);
