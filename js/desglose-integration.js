...
(function() {
  ...
  async function cargarTarifas() {
    ...
    const sharedTarifas = Array.isArray(window.LF?.cachedTarifas) ? window.LF.cachedTarifas : null;
    if (sharedTarifas && sharedTarifas.length > 0) {
      tarifasCache = sharedTarifas;
      return tarifasCache;
    }
    ...
    try {
      ...
      // window.LF.cachedTarifas aun no esta listo, pero debe aplicar el mismo criterio.
      // Fail-closed: sin el validador no se asume que los datos son utilizables.
      const esTarifaUtilizable = window.LF?.esTarifaUtilizable;
      if (typeof esTarifaUtilizable !== 'function') {
        lfDbg('esTarifaUtilizable no disponible; ¿lf-utils.js cargó antes que desglose-integration.js?');
        return [];
      }
      if (!tarifas.every(esTarifaUtilizable)) return [];
      tarifasCache = tarifas;
      return tarifasCache;
    } catch (error) {
  ...
  window.mostrarDesglose = async function(nombreTarifa) {
    ...
    // Las filas del ranking siguen siendo clicables aunque haya cambios sin calcular
    // (state.pending, ver markPending() en lf-ui.js): sin este guard, el desglose lee los
    // inputs del DOM ACTUALES en vez de un snapshot de los que generaron esa fila, pudiendo
    // calcular con datos distintos de los que el usuario ve en el ranking.
    if (window.LF?.state?.pending) {
      toast('Hay cambios pendientes. Pulsa "Calcular" antes de abrir el desglose.', 'err');
      return;
    }
    ...
    if (nombreTarifa === 'Mi tarifa ⭐') {
      ...
      const mtPrecioBVVal = document.getElementById('mtPrecioBV')?.value?.trim() || '';
      ...
      const mtTieneBV = document.getElementById('mtBV')?.checked || false;
      // Cuota mensual de BV: solo se lee/aplica si hay BV activa (mismo criterio que
      // getCustomTarifa() en bv-ui.js y agregarMiTarifa() en lf-tarifa-custom.js).
      const mtPrecioBV = mtTieneBV && mtPrecioBVVal ? parseNum(mtPrecioBVVal) : 0;
      // Opciones avanzadas (14/08/2026): mismo criterio que getCustomTarifa()/agregarMiTarifa.
      const mtSinSSAA = document.getElementById('mtSinSSAA')?.checked || false;
      const mtCompensacionIndexada = inputs.solarOn && (document.getElementById('mtCompensacionIndexada')?.checked || false);
      const mtTopeParcial = inputs.solarOn && (document.getElementById('mtTopeParcial')?.checked || false);
      const mtPrecioExcFinal = mtCompensacionIndexada ? -1 : mtPrecioExc;
      const mtCompensa = mtPrecioExcFinal > 0 || mtPrecioExcFinal === -1;
      ...
      tarifa = {
        ...
        p1: mtP1,
        p2: mtP2,
        incluyeServiciosAjuste: !mtSinSSAA,
        fv: {
      ...
      // CASO ESPECIAL: PVPC. Su desglose ya dispone de metaPvpc calculada y no
      // necesita una segunda descarga de tarifas.json.
      if (nombreTarifa && nombreTarifa.toLowerCase().includes('pvpc')) {
        ...
        // Reutilizar primero la lista que ya cargó el comparador. Solo si no está
        // disponible se recurre a red, evitando que un segundo fallo de tarifas.json
        // bloquee un desglose de resultados ya calculados.
    ...
    const datos = {
      ...
      pvpcCoverage: tarifa.esPVPC ? (window.pvpcLastMeta?.pvpcCoverage || null) : null,
