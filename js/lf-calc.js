...
(function() {
  ...
  const roundMoneyProducts = (CFG && typeof CFG.roundMoneyProducts === 'function')
    ? CFG.roundMoneyProducts
  ...
  function __LF_aplicarImpuestos(params) {
    ...
    const impuestoElec = (typeof CFG.calcularIEERedondeado === 'function')
      ? CFG.calcularIEERedondeado(params.sumaBase, params.consumoTotalKwh, fiscal.fechaYmd)
      : round2(CFG.calcularIEE(params.sumaBase, params.consumoTotalKwh, fiscal.fechaYmd));
  ...
  async function calculateLocal(values) {
    ...
    for (let chunkStart = 0; chunkStart < cachedTarifas.length; chunkStart += CHUNK_SIZE) {
      ...
      for (let index = chunkStart; index < chunkEnd; index++) {
        ...
        const pot = roundMoneyProducts([[p1, dias, t.p1], [p2, dias, t.p2]]);
        const consBase = roundMoneyProducts([[cPunta, t.cPunta], [cLlano, t.cLlano], [cValle, t.cValle]]);
        ...
        if (ssaa.aplica && ssaa.available === false) {
          resultados.push({
            ...
            dataUnavailableReason: 'SSAA temporalmente no disponible: esta tarifa no incluye los servicios de ajuste y no se puede calcular sin omitir ese coste regulado.',
            ssaaUnavailableReason: ssaa.reason || 'dataset-unavailable',
            ssaaRequestedMonth: ssaa.requestedMonth || null,
            solarNoCalculable: false
          });
          continue;
        }
        ...
        if (solarOn && !t.esPVPC) {
          ...
          if (fv && fv.tipo !== 'NO COMPENSA') {
            ...
            if (precioExc === null) {
              ...
              fvApplied = true;
              const creditoPotencial = roundMoneyProducts([[exKwh, precioExc]]);
    ...
    // Una condición contractual cuantificada no puede competir en el ranking
    // cuando los kWh ya introducidos hacen imposible cumplirla. En un periodo
    ...
    if (window.LF.state?.useAnnualConsumptionEstimate) {
      ...
      } else if (window.LF.state.annualConsumptionEstimateBasis !== estimateBasis) {
        window.LF.state.useAnnualConsumptionEstimate = false;
        window.LF.state.annualConsumptionEstimateBasis = null;
      }
    }

    // Filtrar tarifas que requieren FV si usuario no tiene solar, o que no
    // admiten los kWh registrados. Un límite vacío/cero conserva el
    // comportamiento anterior.
    const candidatas = solarOn ? resultados : resultados.filter(r => !r.requiereFV);
    const limitesConsumo = window.LF.assessConsumoAnualLimits(candidatas, {
    ...
    const resultadosFiltrados = limitesConsumo.compatibles;

    // Solo una fila con total finito puede convertirse en "mejor opción". Si
    // todas las tarifas han quedado temporalmente no calculables (p.ej. SSAA
    // obligatorio sin dataset), no inventamos un ganador con precio "—".
    const firstValida = resultadosFiltrados.find(r => Number.isFinite(r.totalNum)) || null;
    const bestPrice = firstValida ? firstValida.totalNum : null;

    let processed = resultadosFiltrados.map((r, i) => {
      const esMejor = Boolean(firstValida && r === firstValida);
    ...
    window.LF.renderAll({
      ...
      limitesConsumo,
      resumen: {
        mejor: firstValida ? firstValida.nombre : '—',
