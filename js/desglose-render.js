...
(function() {
  ...
  Object.assign(Desglose, {
    renderizar(d, datos) {
      ...
      // Promocion: se informa, nunca se aplica al calculo. Por eso lleva
      // la coletilla explicita de que el total de abajo no la incluye.
      const promoEl = this.modal.querySelector('.desglose-promo');
      if (promoEl) {
        if (datos.promo) {
          promoEl.textContent = `🎁 Oferta: ${datos.promo} No incluida en este cálculo.`;
      ...
      const rawPotP1 = d.isPVPC
        ? roundMoneyProducts([[safeNum(datos.potenciaP1), _pvpcPot.p1 ?? 0.075901, diasCalc]])
        : roundMoneyProducts([[safeNum(datos.potenciaP1), diasCalc, safeNum(datos.precioP1)]]);
      const rawPotP2 = d.isPVPC
        ? roundMoneyProducts([[safeNum(datos.potenciaP2), _pvpcPot.p2 ?? 0.001987, diasCalc]])
        : roundMoneyProducts([[safeNum(datos.potenciaP2), diasCalc, safeNum(datos.precioP2)]]);
      ...
      const rawConsP1 = roundMoneyProducts([[safeNum(datos.consumoPunta), safeNum(datos.precioPunta)]]);
      const rawConsP2 = roundMoneyProducts([[safeNum(datos.consumoLlano), safeNum(datos.precioLlano)]]);
      const rawConsP3 = roundMoneyProducts([[safeNum(datos.consumoValle), safeNum(datos.precioValle)]]);
      ...
      html += `<div class="desglose-resumen">
        ...
        ${esCompParcial && compensa ? (() => {
          ...
          const cpP1 = roundMoneyProducts([[safeNum(datos.consumoPunta), pc.P1 || 0]]);
          const cpP2 = roundMoneyProducts([[safeNum(datos.consumoLlano), pc.P2 || 0]]);
          const cpP3 = roundMoneyProducts([[safeNum(datos.consumoValle), pc.P3 || 0]]);
        ...
        ${solarOn ? `<aside class="desglose-solar-simulator" aria-label="Recomendación para autoconsumo">
          <div class="desglose-solar-simulator-icon" aria-hidden="true">☀️</div>
          <div>
            <strong>Para autoconsumo, este ranking es orientativo.</strong>
            <p>Para decidir qué tarifa solar te conviene, usa el <a href="/comparador-tarifas-solares.html">Simulador Solar</a>: calcula mes a mes, arrastra la batería virtual entre facturas, permite elegir el mes de inicio y puede usar tu CSV de consumo.</p>
          </div>
        </aside>` : ''}
      </div>`;
