...
(function() {
  ...
  const {
    ...
    showError, clearErrorStyles, applyButtonState,
    esNumericoValido
  } = window.LF;
  ...
  function getInputValues() {
    ...
    const bonoSocialTipo = getRadioValue('bonoSocialTipo', DEFAULTS.bonoSocialTipo);
    const bonoSocialLimite = clampNonNeg(parseNum(getRadioValue('bonoSocialLimite', DEFAULTS.bonoSocialLimite)));
  ...
  function buildCsvConsumosRef(values) {
    const v = values || getInputValues();
    const profiles = window.LF?.csvUtils?.getCsvZoneProfiles?.(v?.zonaFiscal) || (() => {
      const zonaNorm = String(v?.zonaFiscal || '')
        .toLowerCase()
        .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        .replace(/[^a-z]/g, '');
      return {
        zonaHoraria: zonaNorm.includes('canaria') ? 'canarias' : 'europa-madrid',
        perfilPeriodos: zonaNorm.includes('ceuta') && zonaNorm.includes('melilla')
          ? 'ceuta-melilla'
          : 'general'
      };
    })();
    const zonaHoraria = v?.zonaHoraria || profiles.zonaHoraria;
    const perfilPeriodos = v?.perfilPeriodos || profiles.perfilPeriodos;
    return {
  ...
  function assessCsvConsumosRef(values, ref) {
    if (!ref || typeof ref !== 'object') {
      return {
        matches: false,
        aggregateMatches: false,
        clockProfileMatches: false,
        periodProfileMatches: false
      };
    }
    const current = buildCsvConsumosRef(values);
    const expected = buildCsvConsumosRef(ref);
    const aggregateMatches = current.dias === expected.dias
      && current.cPunta === expected.cPunta
      && current.cLlano === expected.cLlano
      && current.cValle === expected.cValle;
    const clockProfileMatches = current.zonaHoraria === expected.zonaHoraria;
    const periodProfileMatches = current.perfilPeriodos === expected.perfilPeriodos;
    return {
      matches: aggregateMatches && clockProfileMatches && periodProfileMatches,
      aggregateMatches,
      clockProfileMatches,
      periodProfileMatches
    };
  }

  function csvConsumosRefMatches(values, ref) {
    return assessCsvConsumosRef(values, ref).matches;
  }
  ...
  function initSolarInfoModal() {
    ...
    let __solarLockToken = null;
    let __solarFallbackState = null;
    let __solarHideTimer = 0;
    let __solarFocusTimer = 0;
    let __solarOpenSeq = 0;

    function __solarCloseDelay(){
    ...
    function __solarLock() {
      ...
      if (shared && typeof shared.lock === 'function') {
        __solarLockToken = shared.lock('solar-info');
      } else {
        const body = document.body;
        const html = document.documentElement;
        __solarFallbackState = {
    ...
    function __solarUnlock() {
      if (!__solarLocked) return;
      const shared = window.LF?.modalScrollLock;
      if (__solarLockToken && shared && typeof shared.unlock === 'function') {
        shared.unlock(__solarLockToken);
      } else if (__solarFallbackState) {
        if (document.body?.style) document.body.style.overflow = __solarFallbackState.bodyOverflow;
        if (document.documentElement?.style) document.documentElement.style.overflow = __solarFallbackState.htmlOverflow;
        if (document.body) document.body.scrollTop = __solarFallbackState.scrollTop;
      }
      __solarLockToken = null;
      __solarFallbackState = null;
      __solarLocked = false;
    }

    function __solarClose(){
      if (!modalSolarInfo) return;
      const closeSeq = ++__solarOpenSeq;
      if (__solarFocusTimer) {
        clearTimeout(__solarFocusTimer);
        __solarFocusTimer = 0;
      }
      modalSolarInfo.classList.remove('show');
      modalSolarInfo.setAttribute('aria-hidden', 'true');
      if (__solarHideTimer) clearTimeout(__solarHideTimer);
      __solarHideTimer = setTimeout(() => {
        __solarHideTimer = 0;
        if (closeSeq === __solarOpenSeq && !modalSolarInfo.classList.contains('show')) {
    ...
    if (btnSolarInfo && modalSolarInfo && btnCerrarSolarInfo) {
      btnSolarInfo.addEventListener('click', (e) => {
        ...
        // Evitar dobles aperturas y cancelar cualquier cierre visual pendiente.
        if (modalSolarInfo.getAttribute('aria-hidden') === 'false') return;
        const myOpenSeq = ++__solarOpenSeq;
        if (__solarHideTimer) {
          clearTimeout(__solarHideTimer);
          __solarHideTimer = 0;
        }
        ...
        requestAnimationFrame(() => {
          if (myOpenSeq !== __solarOpenSeq || modalSolarInfo.getAttribute('aria-hidden') === 'true') return;
          ...
          __solarFocusTimer = setTimeout(() => {
            __solarFocusTimer = 0;
            if (myOpenSeq === __solarOpenSeq && modalSolarInfo.classList.contains('show')) target?.focus?.();
          }, 0);
        });
      });
  ...
  function loadInputs() {
    if (Object.keys(SERVER_PARAMS).length > 0) {
      ...
      updateSolarUI();
      updateBonoSocialUI();
      // El comportamiento de la UI no puede depender de DE DONDE vengan los valores. Este
      // `return` dejaba sin enganchar el listener de bonoSocialOn (el unico esta en
      // attachInputEventListeners), asi que al abrir una configuracion compartida el
      // checkbox se marcaba pero #bonoSocialFields nunca se mostraba.
  ...
  // Idempotente a proposito: loadInputs la invoca desde sus dos ramas (configuracion
  // compartida por URL y carga desde localStorage), y ademas se marca sobre el propio
  // elemento y no en una variable de modulo para que un remplazo del DOM vuelva a
  // engancharla en vez de quedarse muda.
  function attachInputEventListeners() {
    const cb = el.inputs.bonoSocialOn;
    if (!cb || cb.__lfBonoSocialBound) return;
    cb.__lfBonoSocialBound = true;
  ...
  function validateInputs() {
    ...
    if (!p1Raw) {
      ...
      message = 'La potencia P1 no puede ser negativa.';
      markInvalid(el.inputs.p1);
    } else if (p1Num > (window.LF_CONFIG?.POTENCIA_MAX_KW || 15)) {
      message = `La potencia P1 debe ser ≤ ${window.LF_CONFIG?.POTENCIA_MAX_KW || 15} kW.`;
      markInvalid(el.inputs.p1);
    }
    ...
    if (!message && !p2Raw) {
      ...
      markInvalid(el.inputs.p2);
    } else if (!message && p2Num > (window.LF_CONFIG?.POTENCIA_MAX_KW || 15)) {
      message = `La potencia P2 debe ser ≤ ${window.LF_CONFIG?.POTENCIA_MAX_KW || 15} kW.`;
      markInvalid(el.inputs.p2);
    }
    ...
    if (!message) {
      ...
      // Un periodo con 0 kWh en P1/P2/P3 es valido: aunque no haya energia
      // consumida, siguen existiendo costes fijos (potencia, contador, etc.).
      // Los tres campos siguen siendo obligatorios para distinguir 0 explicito de vacio.
    }
