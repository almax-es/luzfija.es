...
(() => {
  ...
  // Anyo vigente en el calendario del dataset elegido, no en el del navegador: en la
  // frontera de anyo, un navegador en otra zona horaria veia otro anyo (ronda 41).
  // Misma zona que usa el motor para su "hoy"; sin motor se resuelve igual que
  // PVPC_STATS.getDatasetTimeZone, y sin helper de fechas se conserva el reloj local.
  function getDatasetCurrentYear(context = {}, now = new Date()) {
  ...
  function getAvailableYearsDesc(today = new Date(), context = {}) {
    const currentYear = getDatasetCurrentYear(context, today);
    const years = [];
    for (let y = currentYear; y >= DATASET_MIN_YEAR; y--) years.push(y);
    return years;
  }

  function populateYearSelector(select, today = new Date(), context = {}) {
    if (!select) return [];
    const years = getAvailableYearsDesc(today, context);
  ...
  function parseParams() {
    ...
    // El tope del anyo depende de la zona del dataset, asi que tipo y geo van primero.
    const type = rawType === 'surplus' || rawType === 'pvpc' ? rawType : 'pvpc';
    const geo = Object.prototype.hasOwnProperty.call(geoNames, rawGeo) ? rawGeo : '8741';
    const currentYear = getDatasetCurrentYear({ geo, type });
    const defaults = {
    ...
    return {
      type,
      geo,
      year: Number.isInteger(yearNumber) && yearNumber >= DATASET_MIN_YEAR && yearNumber <= currentYear
        ? String(yearNumber)
        : defaults.year,
  ...
  function applyStateToControls(state) {
    ...
    populateYearSelector(els.year, new Date(), state);
  ...
  function attachControlHandlers(state, rerender) {
    ...
    // Zona y tipo cambian el calendario del dataset: el anyo elegido se ajusta ANTES de pedir datos.
    const onDatasetChange = () => {
      alignStateToDataset(state);
      populateYearSelector(els.year, new Date(), state);
      els.year.value = state.year;
      onChange();
    };
    if (els.type) els.type.addEventListener('change', () => { state.type = els.type.value; onDatasetChange(); });
    els.geo.addEventListener('change', () => { state.geo = els.geo.value; onDatasetChange(); });
  ...
  // Canarias cambia de anyo una hora despues que la Peninsula. A las 00:30 de Madrid del 01/01,
  // pasar de Peninsula 2027 a Canarias dejaba 2027 elegido y el motor no pedia ningun mes: la vista
  // quedaba vacia y sin explicacion (ronda 41, C41-02). El anyo y los comparados se limitan al
  // anyo en curso del dataset nuevo; si no sobrevive ningun comparado, se recalculan.
  function alignStateToDataset(state, now = new Date()) {
    const currentYear = getDatasetCurrentYear(state, now);
    if (!(Number(state.year) <= currentYear)) state.year = String(currentYear);
    const compared = (state.compareYears || []).filter((y) => y <= currentYear);
    state.compareYears = compared.length
      ? compared
      : normalizeSelectedYears(state.year, '', state);
    return state;
  }

  function normalizeSelectedYears(year, compareYearsParam, context = {}) {
    const currentYear = getDatasetCurrentYear(context);
  ...
  async function main() {
    ...
    const currentSystemYear = String(getDatasetCurrentYear(params));
    ...
    const state = {
      ...
      compareYears: normalizeSelectedYears(params.year || currentSystemYear, params.compareYears, params)
    };
    ...
    const rerender = debounce(async ({ push = false } = {}) => {
      ...
      const isCurrentYear = String(state.year) === String(getDatasetCurrentYear(state));
      ...
      // Un chip del render anterior sigue activo mientras este carga: si se pulso tras un cambio
      // de zona, pudo reintroducir un anyo que el dataset actual aun no ha empezado (ronda 41).
      const comparedBefore = state.compareYears.join(',');
      alignStateToDataset(state);
      if (state.compareYears.join(',') !== comparedBefore) writeParams(state, { replace: true });
      const currentYear = getDatasetCurrentYear(state);
      const allYears = [];
      for (let y = currentYear; y >= 2021; y--) allYears.push(y);

      const toggleYear = async (y) => {
        // Mismo caso desde el propio chip obsoleto: su lista de anyos es la del dataset anterior.
        if (y > getDatasetCurrentYear(state)) return;
