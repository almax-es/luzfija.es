...
(function() {
  ...
  function rememberInitRecovery(detail) {
    ...
    const alreadyPending = pending.some((item) => item &&
      item.app === entry.app && item.dependency === entry.dependency && item.build === entry.build);
  ...
  window.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
      ...
      const solarTooltip = target.closest('#tbody .fv-icon, #tbody .consumo-limits-icon, #tbody .requisitos-icon, #tbody .te-warn-icon, #tbody .promo-badge');
      if (solarTooltip) {
  ...
  window.addEventListener('error', function(e) {
    try{
      ...
      const errorLabel = scriptSource
        ...
        : (resource ? 'Carga de recurso fallida: ' + resource.kind : message.substring(0, 48));
      const parts = [
        errorLabel,
        source + ':' + line + (col ? ':' + col : ''),
        'b:' + TRACK_BUILD_ID
