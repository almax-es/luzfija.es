...
(() => {
  ...
  async function parseCsvOrXlsx(file, zonaFiscal = 'Península') {
    ...
    if (ext === 'xlsx' || ext === 'xls') {
      ...
      // Camino inalcanzable hoy (verificado 17/09/2026, ronda 43): con UNA candidata la
      // consume el parser comun por `mapSafeFallbackSolarExport`, y con dos o mas el guard de
      // abajo cancela la importacion. Se conserva como red de seguridad, pero cuenta lo que
      // descarta: es un parser paralelo y sin esto reintroduciria la perdida silenciosa que
      // corrigio la ronda 43.
      const fallbackRecords = [];
      let fallbackDescartadas = 0;
      ...
      if (fallbackRecords.length) {
        ...
        const avisos = ['Importación XLSX: aplicado parser alternativo para excedentes.'];
        if (fallbackDescartadas > 0) {
          avisos.push(`Se descartaron ${fallbackDescartadas} filas con fecha u hora no reconocidas o excedentes no numéricos.`);
        }
        return { records: fallbackRecords, warnings: avisos };
      }
      abortarSiQuedaColumnaSolar();
      return parsed;
    }
