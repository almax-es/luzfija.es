...
(function () {
  ...
  // Un mes es utilizable solo si CADA dia declarado cubre su dia civil completo (misma
  // regla que home y Observatorio, via el validador compartido de lf-csv-utils.js). El
  // dia local vigente (segun la zona del propio dataset) puede estar parcial: ESIOS aun
  // no ha terminado de publicarlo. Antes solo se comprobaba "cada fila es un par
  // numerico", asi que un dia con un unico punto horario pasaba como sano (bloqueante 2,
  // 12/08/2026).
  ...
  async function loadSurplusMonth(geo, ym) {
    ...
    const promise = (async () => {
      try {
        ...
        // No hacer negative-cache de fallos de red/HTTP: si la conexión se
        // recupera, el siguiente cálculo de la sesión debe poder reintentarlo.
        return null;
      } finally {
  ...
  function summarizeCompensation({ monthly, monthlyHourly, hourly, totalKwh, totalEur, missing, totalMissingKwh, totalPricedHours }) {
    ...
    const monthlyRows = monthsOrdered.map((ym) => {
      ...
      const creditEur = roundMoneyProductsOrFallback(row.moneyProducts || [], row.eur);
      return {
        ym,
        kwh: row.kwh,
        eur: creditEur,
        // Sin kWh valorados no existe un precio medio ponderado. `0` sería un dato
        // inventado que la tabla del Observatorio acabaría mostrando como 0 €/kWh.
        avg: row.kwh ? row.eur / row.kwh : null,
        missing,
        missingKwh,
        inputKwh: row.kwh + missingKwh,
        pricedHours,
    ...
    // El precio medio conserva la suma horaria cruda. El KPI monetario, en cambio,
    // representa créditos liquidados por mes: suma las filas ya normalizadas a
    // céntimos para que coincida con el detalle visible y no reintroduzca IEEE-754.
    ...
    // Un mes sin una sola hora valorada no tiene precio medio: `avg=0` es solo el
    // valor neutro interno de la fila. Incluirlo en mejor/peor hacía que una falta
    // completa de cobertura apareciese en la UI como un precio real de 0 €/kWh
    // (y con precios negativos podía incluso figurar como el "mejor" mes).
    const comparableMonthlyRows = monthlyRows.filter((row) => row.pricedHours > 0 && row.kwh > 0);
    ...
    return {
      totalKwh,
      inputKwh: totalKwh + (totalMissingKwh || 0),
  ...
  async function computeHourlyCompensation(records, options = {}) {
    ...
    function markMissing(ym, missingKwh = 0) {
      ...
      missing += 1;
      totalMissingKwh += kwh;
      if (!monthly[ym]) monthly[ym] = { kwh: 0, eur: 0, moneyProducts: [], missing: 0, missingKwh: 0, pricedHours: 0 };
