...
(function() {
  ...
  function assertRelevantXlsxFormulasResolved(sheet, rows, headerRowIndex, options = {}) {
    ...
    const headerRow = Array.isArray(rows[headerRowIndex]) ? rows[headerRowIndex] : [];
    // `rows`, `headerRowIndex` y las columnas relevantes se cuentan desde el inicio del rango
    // (!ref), como las devuelve sheet_to_json; las direcciones de celda son absolutas. Sin restar
    // ese inicio, una hoja que empieza en B2 miraba la columna de al lado: dejaba pasar una formula
    // sin resultado en el consumo, que entraba como 0 kWh, y podia rechazar el fichero por una
    // formula en una columna que no importa (16/09/2026).
    const inicio = __LF_xlsxColumnIndexFromAddress(String(sheet['!ref'] || '').split(':')[0])
      || { col: 0, row: 0 };

    for (const [address, cell] of Object.entries(sheet)) {
      if (!cell || typeof cell !== 'object' || !cell.f) continue;
      const celda = __LF_xlsxColumnIndexFromAddress(address);
      if (!celda) continue;
      const pos = { col: celda.col - inicio.col, row: celda.row - inicio.row };
      if (pos.row <= headerRowIndex || !relevant.has(pos.col)) continue;
      ...
      // La fila del mensaje es la de Excel, la que el usuario ve en su hoja.
      throw buildImportError(
        `La hoja contiene una fórmula sin resultado calculado en "${label}" (fila ${celda.row + 1}). ` +
  ...
  function parseEnergyTableRows(rows, options = {}) {
    ...
    // Filas descartadas por su contenido. La matriz horaria ya avisaba de sus descartes; esta
    // ruta los omitia en silencio y, por debajo del umbral del 50 %, un fichero podia perder
    // una parte de su consumo sin que nadie lo supiera (ronda 43).
    let fechaHoraInvalidaCount = 0;
    let noNumericoCount = 0;
    let negativoCount = 0;
    ...
    for (const row of dataRows) {
      ...
      if (!fecha || !Number.isFinite(hourNum)) {
        ...
        const tiempoVacio = mapping.fechaHoraIdx !== null && mapping.fechaHoraIdx !== undefined
          ? isEmptyCell(row[mapping.fechaHoraIdx])
          : isEmptyCell(row[mapping.fechaIdx]) && isEmptyCell(row[mapping.horaIdx]);
        if (!tiempoVacio) fechaHoraInvalidaCount++;
        continue;
      }
      const hora = resolveHour(fecha, hourNum, mapping.invVerIdx !== null ? row[mapping.invVerIdx] : null);
      if (!Number.isFinite(hora) || hora < 1 || hora > 25) {
        fechaHoraInvalidaCount++;
        continue;
      }
    ...
    if (totalRows > 0 && parsedRows / totalRows < 0.5) {
      // Si los contadores explican el rechazo, se dice la causa en vez de culpar al separador.
      // Redaccion elegida para que csvErrorCodeForTracking siga dando 'filas-invalidas': nada
      // de "valores no numericos" ni "columna", que casan antes con otros codigos.
      const causas = [
        [fechaHoraInvalidaCount, 'con fecha u hora no reconocidas'],
        [noNumericoCount, 'con un consumo o excedente que no es un número'],
        [negativoCount, 'con valores negativos'],
        [outOfRangeCount, 'con valores superiores a 10.000 kWh'],
        [h25DiscardedCount, 'con hora 25 fuera del cambio de hora de octubre']
      ].filter(([n]) => n > 0);
      const explicadas = causas.reduce((acc, [n]) => acc + n, 0);
      const mensaje = explicadas > 0 && explicadas * 2 >= totalRows - parsedRows
        ? `La mayoría de filas no se pudo interpretar: ${causas.map(([n, texto]) => `${n} ${texto}`).join(', ')}.`
        : 'La mayoría de filas no se pudo interpretar; probable separador o cabecera incorrecta.';
      throw buildHeaderError(mensaje, headersNorm, { separator });
    }
    ...
    if (fechaHoraInvalidaCount > 0) {
      warnings.push(`Se descartaron ${fechaHoraInvalidaCount} filas con fecha u hora no reconocidas.`);
    }
    if (noNumericoCount > 0) {
      warnings.push(`Se descartaron ${noNumericoCount} filas con valores no numéricos.`);
    }
    if (negativoCount > 0) {
      warnings.push(`Se descartaron ${negativoCount} filas con valores negativos.`);
    }
