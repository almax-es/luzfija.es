...
(function() {
  ...
  // En el comparador de la home los excedentes son opcionales: un fichero de solo consumo
  // es perfectamente valido. Una columna solar sin reconocer se avisa, nunca bloquea.
  const UNMAPPED_SOLAR_POLICY = 'warn';
  ...
  async function parseXLSXConsumos(fileBuffer) {
    ...
    if (typeof assertXlsxSheetWithinLimits !== 'function') {
      throw buildImportError('No se pudo validar el archivo Excel; inténtalo de nuevo.');
    }
    ...
    // La implementación vive en lf-csv-utils.js y la comparten home y solar: antes estaba
    // duplicada en los dos ficheros con políticas distintas para los valores negativos.
    const {
    ...
    if (matrixHeaderRow !== -1) {
      assertRelevantXlsxFormulasResolved(firstSheet, data, matrixHeaderRow, { format: 'matrix' });
      return parseHourlyMatrixRows(data, matrixHeaderRow, {
        parseNumber: parseNumberFlexible,
        zonaFiscal: window.LF?.getInputValues?.()?.zonaFiscal || 'Península'
      });
    }
    ...
    assertRelevantXlsxFormulasResolved(firstSheet, data, headerRow);
  ...
  function applySpanValidation(consumos, warnings, options = {}) {
    ...
    if (typeof validateCsvSpanFromRecords !== 'function') {
      return { ok: true, consumos, warnings: nextWarnings, span: null };
    }
    ...
    const span = {
      spanDays: Number(spanCheck.spanDays) || 0,
      startYmd: spanCheck.startYmd || '',
      endYmd: spanCheck.endYmd || ''
    };
    ...
    if (Array.isArray(spanCheck.monthsToDrop) && spanCheck.monthsToDrop.length > 0) {
      ...
      return { ok: true, consumos: filtered, warnings: nextWarnings, span };
    }
    return { ok: true, consumos, warnings: nextWarnings, span };
  }

  // ===== CLASIFICAR CONSUMOS =====
  function clasificarConsumosPorPeriodo(consumos, options = {}) {
    const totales = {
      P1: 0, P2: 0, P3: 0,
    ...
    const zonaFiscal = options.zonaFiscal || window.LF?.getInputValues?.()?.zonaFiscal || 'Península';
  ...
  function reclasificarConsumosHorarios(consumos, zonaFiscal) {
    if (!Array.isArray(consumos) || consumos.length === 0) {
      return { ok: false, error: 'No hay curva horaria que reclasificar.' };
    }
    const records = consumos.map((record) => ({
      ...record,
      periodo: getPeriodoHorarioCSV(record.fecha, record.hora, zonaFiscal)
    }));
    return {
      ...clasificarConsumosPorPeriodo(records, { zonaFiscal }),
      records
    };
  }

  function buildCoverageDiscontinuityMessage(resultado) {
    if (!resultado || resultado.isDatadisMonthly) return '';
    const spanDays = Number(resultado.spanDays);
    const daysWithData = Number(resultado.dias);
    if (!Number.isFinite(spanDays) || !Number.isFinite(daysWithData) || spanDays <= daysWithData) return '';

    const start = String(resultado.spanStartYmd || '').trim();
    const end = String(resultado.spanEndYmd || '').trim();
    const range = start && end ? ` (${start} → ${end})` : '';
    return `El archivo abarca ${spanDays} días de calendario${range}, pero contiene lecturas en ${daysWithData} días. ` +
      `LuzFija mantendrá ${daysWithData} días en el campo «Días», que es el valor usado para los cargos diarios. ` +
      'Revisa que el archivo no esté incompleto antes de aplicar los consumos.';
  }
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          resultado.spanDays = spanResult.span?.spanDays || resultado.dias;
          resultado.spanStartYmd = spanResult.span?.startYmd || '';
          resultado.spanEndYmd = spanResult.span?.endYmd || '';
          ...
          // Los avisos se pintan dentro de la vista previa (buildImportNoticesHTML).
  ...
  // ===== MODAL PREVIEW =====
  // Avisos del parser dentro de la vista previa. Antes se lanzaban como toast justo antes de
  // abrir el modal, y el toast queda debajo del overlay (z-index 10010): el usuario no llegaba
  // a ver ninguno, ni los de filas descartadas ni los de neteo, Wh o cambio de hora (ronda 43,
  // medido en Chrome). Pueden citar cabeceras del archivo: siempre escapados.
  function buildImportNoticesHTML(warnings) {
    const lista = (Array.isArray(warnings) ? warnings : [])
      .map((w) => String(w ?? '').trim())
      .filter(Boolean);
    if (!lista.length) return '';
    const escape = typeof window.LF?.escapeHtml === 'function'
      ? window.LF.escapeHtml
      : (v) => String(v ?? '').replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' })[c]);
    const items = lista
      .map((w) => `<li style="margin:0 0 4px 0; white-space:pre-line;">${escape(w)}</li>`)
      .join('');
    return `<div id="csvImportNotices" role="note" style="margin-bottom:16px; padding:12px 14px; border:1px solid var(--border); border-radius:10px; background:rgba(99,102,241,.08); color:var(--text); font-size:12px; line-height:1.5;"><strong>Avisos al leer el archivo</strong><ul style="margin:6px 0 0 0; padding-left:18px;">${items}</ul></div>`;
  }
  ...
  function mostrarPreviewCSV(resultado) {
    ...
    function __csvUnlock() {
      if (!__csvLocked) return;
      const shared = window.LF?.modalScrollLock;
      if (__csvLockToken && shared && typeof shared.unlock === 'function') {
        shared.unlock(__csvLockToken);
      } else if (__csvFallbackState) {
        if (document.body?.style) document.body.style.overflow = __csvFallbackState.bodyOverflow;
        if (document.documentElement?.style) document.documentElement.style.overflow = __csvFallbackState.htmlOverflow;
        if (document.body) document.body.scrollTop = __csvFallbackState.scrollTop;
      }
      __csvLockToken = null;
      __csvFallbackState = null;
      __csvLocked = false;
    }
    ...
    const discontinuityMessage = buildCoverageDiscontinuityMessage(resultado);
    const discontinuityHTML = discontinuityMessage
      ? `<div id="csvDiscontinuityNotice" role="note" style="margin-bottom:16px; padding:12px 14px; border:1px solid var(--warn); border-radius:10px; background:rgba(245,158,11,.10); color:var(--text); font-size:12px; line-height:1.5;"><strong>⚠️ Cobertura discontinua.</strong> ${window.LF.escapeHtml ? window.LF.escapeHtml(discontinuityMessage) : discontinuityMessage}</div>`
      : '';
    ...
    content.innerHTML = `
      ...
      ${discontinuityHTML}
      ${buildImportNoticesHTML(resultado.warnings)}
      ...
      btnAplicar.addEventListener('click', () => {
        ...
        window.LF.csvConsumosRef = importedCurve
          ? window.LF.buildCsvConsumosRef({
              ...
              zonaFiscal: window.LF?.getInputValues?.()?.zonaFiscal
            })
          : null;
  ...
  window.LF.csvHelpers = {
    ...
    parseCSVConsumos,
    buildCoverageDiscontinuityMessage,
    buildImportNoticesHTML
  };
