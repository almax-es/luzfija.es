...
- Antes de actualizar cualquier dependencia de `vendor/`, lee `vendor/README.md`: tiene version, SHA-256 y procedimiento por libreria. Dos trampas ya documentadas ahi que NO hay que volver a pisar: (1) `vendor/goatcounter/count.js` no es upstream limpio, lleva tres parches locales y sobrescribirlo con `curl` los destruye en silencio -usar la linea base `count.upstream.js` para el merge-; (2) el dist-tag `latest` de `tesseract.js-core` apunta a la linea 6.x, pero el wrapper 7.x exige core `^7.0.0`, asi que "actualizar a latest" seria una regresion.
