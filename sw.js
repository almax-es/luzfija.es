...
const CACHE_VERSION = "20260826-135432";
...
// build anterior y la red falla justo al pedirlos, permitir su copia cacheada
// mantiene el watchdog/recarga. Ningun modulo funcional comparte esta excepcion.
const CROSS_BUILD_RECOVERY_PATHS = new Set([
  new URL("js/error-bootstrap.js", SCOPE).pathname,
  new URL("js/lf-sw-update.js", SCOPE).pathname
]);
...
function isTransientHttpFailure(response) {
  return Boolean(response && (response.status === 408 || response.status === 429 || response.status >= 500));
}

async function matchHealthyCache(cache, req) {
  try {
...
// El precache guarda "estadisticas/index.html" y "guias/index.html", pero el
// usuario navega a "/estadisticas/" y "/guias/". Esas claves no coinciden, asi
// que sin normalizar la ruta de directorio la navegacion offline se saltaba su
// propia copia y caia al INDEX_PATH raiz, sirviendo la home en su lugar.
...
self.addEventListener("fetch", (event) => {
  ...
  // Sonda diagnóstica: red pura, sin leer ni escribir Cache Storage. Debe ir
  // antes que todas las estrategias por destination/pathname porque fetch()
  // crea peticiones con destination="" y, sin esta rama, mediría el fallback
  // del propio SW en lugar del estado de la red.
  if (url.searchParams.get("__lfprobe") === "1") {
    event.respondWith(
    ...
    event.respondWith(
      (async () => {
        ...
        try {
          ...
          // Recursos versionados (?v=...): un SW de otro build no puede
          // satisfacer silenciosamente una pagina nueva con JS/worker viejo (ni
          // una pagina vieja con bytes nuevos de su cache activa). Si la version
          // solicitada no coincide con la identidad de este worker, fallar cerrado:
          // error-bootstrap / los guards funcionales podran pedir recuperacion.
          //
          // Para el MISMO build usamos ignoreSearch. El precache guarda first-party
          // con URL estable sin ?v= y los vendors lazy (p.ej. PDF.js) pueden quedar
          // cacheados en runtime con ?v=. Ambas representaciones pertenecen a esta
          // CACHE_NAME y deben seguir disponibles offline.
          if (url.search) {
            const requestedBuild = url.searchParams.get("v");
            const executableResource = req.destination === "script" || req.destination === "worker";
            if (executableResource && requestedBuild && requestedBuild !== CACHE_VERSION &&
                !CROSS_BUILD_RECOVERY_PATHS.has(url.pathname)) {
              return Response.error();
            }
            return (await matchHealthyCache(cache, req)) || Response.error();
          }
          return (await matchHealthyCache(cache, req)) || Response.error();
        }
      })()
