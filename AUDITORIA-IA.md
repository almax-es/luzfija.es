...
- Desde agosto de 2026, `init-incompleto` y los fallos tempranos de scripts
  ...
  comparacion de build por `GET_VERSION` y aviso persistente con recarga
  explicita. No confundas esa cola efimera `{ app, dependency, build }` con el
...
- El timeout de `lf-cache.js` cubre tambien la lectura de `response.json()`. Si
  ...
  como `json-parse`. Un JSON sintacticamente valido pero sin tarifas es
