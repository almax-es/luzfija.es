...
**Última actualización del inventario:** 03/08/2026

**Última revisión documentada de versiones y vulnerabilidades:** 03/08/2026 — sin advisories conocidas para ninguna de las versiones vendorizadas (consultado el endpoint de advisories del registro npm para las versiones exactas de la tabla). Es una comprobación fechada, no una garantía permanente.
...
| GoatCounter | upstream + 2 parches | idéntico | ✅ al día |
...
- **Versión:** `count.js` upstream + **dos** parches locales (privacidad y confirmación de entrega). Línea base verificada contra upstream el **03/08/2026**.
...
- **Parches locales (son DOS, hay que reaplicar LOS DOS):**
...
- **Red de seguridad (tests):**
  ...
  - `tests/tracking-privacy.test.js` — comportamiento real de los parches: `el count.js local no envía query completa con configuraciones o búsquedas` (parche 1), `el sender confirma beacon y notifica el resultado del fallback de imagen` y `force_image evita dar por entregado un diagnóstico solo porque beacon lo aceptó` (parche 2).
- **Licencia:** ISC
- **Archivos:**
  - `goatcounter/count.js` (servido)
    - **SHA-256:** `05ff41d716390cbec3a5ab1e491023db6719d936978f670315d0883b62f2334d`
    - **Tamaño:** 10.21 KB (10.452 bytes)
  ...
  - `goatcounter/count.local.patch` (los dos parches locales congelados, **no se sirve**)
    - **SHA-256:** `dd7f0ac19e69354f246f41fa73d829acbeeb8fee06783311c05aa5e2b9c4eb24`
    - **Tamaño:** 2.66 KB (2.723 bytes)
