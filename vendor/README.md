...
- **Texto en Safari:** incluso la build `legacy` 6.3.289 implementa `getTextContent()` con `for await...of` sobre un `ReadableStream`. Safari/iOS hasta 26.5 no expone el iterador asíncrono de Web Streams aunque sí `getReader()`. `js/factura.js` agrega directamente los chunks de `streamTextContent().getReader()` y conserva `getTextContent()` solo como fallback de compatibilidad; no retirar ese workaround hasta que la versión vendorizada deje de depender del iterador ausente.
