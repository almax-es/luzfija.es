...
Tras la descarga, el paso `Verificar frescura de datos` ejecuta `scripts/check_data_freshness.py` (primero su self-test con fixtures sintéticos, después el chequeo real): si el dato más reciente de PVPC supera 1 día de antigüedad, el de Excedentes 2 días o el de SSAA 2 meses, el workflow falla y GitHub notifica al propietario, en vez de terminar en verde con datos rancios (por ejemplo, si ESIOS responde vacío sin error).

Después, si hay cambios:
...
**`scripts/check_data_freshness.py`** — Guardia de frescura (los tres datasets)

```
Umbrales: PVPC ultimo dia >= hoy-1 · Excedentes >= hoy-2 · SSAA mes 'to' >= mes actual-2
Exit 1 con listado de datasets rancios/ilegibles → el workflow falla y GitHub notifica.
--self-test: valida el propio guard con fixtures sinteticos (datos caducados, JSON
invalido, directorios vacios); pvpc.yml lo ejecuta antes del chequeo real.
...
Formato real (firma):
pvpc_cache_v1:{anchorDate}:{zona}:{codigoPostal}:{viviendaCanarias}:{p1}:{p2}:{dias}:{cPunta}:{cLlano}:{cValle}:{bonoSocialOn}:{bonoSocialTipo}:{bonoSocialLimite}:{csvSignature}
...
**Solución**:
```javascript
// En consola: eliminar todas las entradas de caché PVPC (prefijo pvpc_cache_v1)
Object.keys(localStorage)
  .filter((k) => k.startsWith('pvpc_cache_v1'))
