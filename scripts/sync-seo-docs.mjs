...
function getAssistantReferenceLastmod(relPath, existingLastmod) {
  const declaredDate = readUtf8(relPath).match(/^Last updated:\s*(\d{4}-\d{2}-\d{2})\s*$/m)?.[1];
  // Las referencias LLM se revisan editorialmente: su fecha declarada no debe
  // avanzar solo porque cambie otro asset incluido en el snapshot global.
  return declaredDate || getSitemapLastmod(relPath, existingLastmod);
}
...
function walkHtmlFiles(dirPath) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (HTML_SKIP_DIRS.has(entry.name) || entry.name.startsWith('.codex-')) continue;
    ...
    const assistantRefs = ['llms.txt', 'llms-full.txt']
      ...
      .map((relPath) => {
        ...
        return {
          ...
          lastmod: getAssistantReferenceLastmod(relPath, existing.lastmod),
...
function countTestMetrics() {
  ...
  if (fs.existsSync(VITEST_SUMMARY_PATH)) {
    try {
      ...
      // Vitest usa numTotalTestSuites para bloques describe, no para archivos.
      // testResults sí contiene una entrada por fichero ejecutado; exigir la
      // misma cantidad evita publicar métricas de una ejecución parcial.
      if (summary?.success === true && Array.isArray(summary?.testResults) &&
          summary.testResults.length === files.length && Number.isInteger(summary?.numTotalTests)) {
    ...
    },
    {
      ...
      apply(content) {
        ...
        if (tests.caseCount !== null) {
          next = next.replace(
            /- \d+ casos `it\(\)\/test\(\)`[^\n]*\./,
            `- ${tests.caseCount} casos \`it()/test()\` en la ultima ejecucion completa verificada.`
          );
        }
        return next;
      }
