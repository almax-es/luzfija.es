...
Los datos de enero siguen siendo los datos de enero del CSV y conservan su clave `YYYY-MM`
original. Se procesan despues de diciembre solo porque el orden elegido determina el arrastre del
saldo BV; no representan una prediccion del enero siguiente. Mantener la clave permite que SSAA y
fiscalidad usen el dato regulado realmente publicado para esa casilla mensual, sin inventar kWh,
excedentes, meses ni años.
...
**Implementación** (simplificada; en la carga productiva normal todos los motores delegan en
`LF_CONFIG`):
```javascript
const impuesto = LF_CONFIG.calcularImpuestoIndirecto({
  zona,
  usoFiscal,
  baseEnergia,
  impuestoElectrico,
...
La funcion comun selecciona IVA/IGIC/IPSI, normaliza las bases monetarias a centimos y aplica el
tipo con aritmetica entera. Home, PVPC, desglose y simulador BV no deben reimplementar el porcentaje.
`lf-utils.js` y `bv-sim-monthly.js` conservan fallbacks defensivos con la antigua aritmetica flotante
si el helper no existe. No son la referencia fiscal: el HTML productivo carga `lf-config.js` antes,
y para considerarlos un bug observable hay que demostrar una carga parcial que llegue a mostrar un
importe calculado por ellos.

---

### ¿Por qué `round2(base * tipo)` y `Number.EPSILON` no bastan siempre?

Porque el error puede aparecer en la multiplicacion antes de que `round2()` reciba el valor. Una
base decimal exacta de `142,50 EUR` al 3% da `4,275 EUR`, que segun el contrato del proyecto debe
redondearse a `4,28 EUR`. En binario, JavaScript puede producir `4.2749999999999995`; sumar un
`Number.EPSILON` generico no garantiza cruzar la frontera y puede dejar `4,27 EUR`.

El motor fiscal evita depender de esa aproximacion: convierte primero la base monetaria a centimos
enteros, representa el tipo en puntos basicos y redondea el cociente entero. Esto se aplica a IVA,
IGIC e IPSI desde `LF_CONFIG.calcularImpuestoIndirecto()`.

Dos motores que coinciden no demuestran por si solos que el centimo sea correcto. La prueba debe
compararlos tambien con una referencia decimal exacta. `tests/fiscal-rounding-align.test.js` fija
las fronteras y una factura real que recorre home, BV y desglose.
...
- [ ] ¿Consulté suites de referencia (`tests/pvpc.test.js`, `tests/fiscal.test.js`, `tests/fiscal-rounding-align.test.js`, `tests/bv-fiscal-align.test.js`)?
- [ ] Si comparé motores, ¿contrasté también el resultado con una referencia decimal independiente?
